...
public class ApiDDictValueControllerTest {
    ...
    public void getOrCreateSingleCodeTableNew() throws Exception{
    	...
    	String json = codeTable.toJson();
    	MockHttpServletResponse response = this.mockMvc.perform(post("/api/v1/ddictvalues/codetable?createTypeKind=true")
    			.contentType(MediaType.APPLICATION_JSON)
    			.accept(MediaType.APPLICATION_JSON)
    			.content(json))
    			.andExpect(status().isCreated())
    ...
    public void getOrCreateCodeTableArray() throws Exception{
    	...
    	MockHttpServletResponse response = this.mockMvc.perform(post("/api/v1/ddictvalues/codetable/jsonArray?createTypeKind=true")
    			.contentType(MediaType.APPLICATION_JSON)
    			.accept(MediaType.APPLICATION_JSON)
    			.content(json))
    			.andExpect(status().isCreated())
    	...
    	for (CodeTableDTO savedCodeTable : savedCodeTables){
    		logger.info(savedCodeTable.toJson());
        	Assert.assertTrue(codeTable.getCodeType().equals(savedCodeTable.getCodeType()) || codeTable2.getCodeType().equals(savedCodeTable.getCodeType()));
        	Assert.assertTrue(codeTable.getCodeKind().equals(savedCodeTable.getCodeKind()) || codeTable2.getCodeKind().equals(savedCodeTable.getCodeKind()));
        	Assert.assertTrue(codeTable.getCode().equals(savedCodeTable.getCode()) || codeTable2.getCode().equals(savedCodeTable.getCode()));
    	}
    }
}
