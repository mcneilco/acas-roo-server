...
@Transactional
@Configurable
@RooIntegrationTest(entity = StateType.class)
public class StateTypeIntegrationTest {
    ...
    public void testFindAllStateTypes() {
        Assert.assertNotNull("Data on demand for 'StateType' failed to initialize correctly", dod.getRandomStateType());
        long count = StateType.countStateTypes();
        Assert.assertTrue("Too expensive to perform a find all test for 'StateType', as there are " + count + " entries; set the findAllMaximum to exceed this value or set findAll=false on the integration test annotation to disable the test", count < 250);
    ...
    public void testFindStateTypeEntries() {
        ...
        Assert.assertEquals("Find entries method for 'StateType' returned an incorrect number of entries", count, result.size());
    }
    ...
    public void testFlush() {
        ...
        obj.flush();
        Assert.assertTrue("Version for 'StateType' failed to increment on flush directive", (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    ...
    public void testMergeUpdate() {
        ...
        Assert.assertTrue("Version for 'StateType' failed to increment on merge and flush directive", (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
