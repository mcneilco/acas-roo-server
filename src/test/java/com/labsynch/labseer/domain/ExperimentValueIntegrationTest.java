...
public class ExperimentValueIntegrationTest {
    ...
    public void testCountExperimentValues() {
        Assert.assertNotNull("Data on demand for 'ExperimentValue' failed to initialize correctly",
                dod.getRandomExperimentValue());
    ...
    public void testFindAllExperimentValues() {
        Assert.assertNotNull("Data on demand for 'ExperimentValue' failed to initialize correctly",
                dod.getRandomExperimentValue());
        long count = ExperimentValue.countExperimentValues();
        Assert.assertTrue("Too expensive to perform a find all test for 'ExperimentValue', as there are " + count
    ...
    public void testFindExperimentValueEntries() {
        ...
        Assert.assertEquals("Find entries method for 'ExperimentValue' returned an incorrect number of entries", count,
                result.size());
    }
    ...
    public void testFlush() {
        ...
        obj.flush();
        Assert.assertTrue("Version for 'ExperimentValue' failed to increment on flush directive",
    ...
    public void testMergeUpdate() {
        ...
        Assert.assertEquals("Identifier of merged object not the same as identifier of original object", merged.getId(),
                id);
        Assert.assertTrue("Version for 'ExperimentValue' failed to increment on merge and flush directive",
                (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    ...
    public void testRemove() {
        ...
        obj.remove();
        obj.flush();
        Assert.assertNull("Failed to remove 'ExperimentValue' with identifier '" + id + "'",
                ExperimentValue.findExperimentValue(id));
    }
}
