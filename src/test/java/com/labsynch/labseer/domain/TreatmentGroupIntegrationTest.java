...
@Transactional
@RooIntegrationTest(entity = TreatmentGroup.class)
public class TreatmentGroupIntegrationTest {
    ...
    public void testCountTreatmentGroups() {
        Assert.assertNotNull("Data on demand for 'TreatmentGroup' failed to initialize correctly", dod.getRandomTreatmentGroup());
    ...
    public void testFindAllTreatmentGroups() {
        Assert.assertNotNull("Data on demand for 'TreatmentGroup' failed to initialize correctly", dod.getRandomTreatmentGroup());
        long count = TreatmentGroup.countTreatmentGroups();
        Assert.assertTrue("Too expensive to perform a find all test for 'TreatmentGroup', as there are " + count + " entries; set the findAllMaximum to exceed this value or set findAll=false on the integration test annotation to disable the test", count < 250);
    ...
    public void testFindTreatmentGroupEntries() {
        ...
        Assert.assertEquals("Find entries method for 'TreatmentGroup' returned an incorrect number of entries", count, result.size());
    }
    ...
    public void testFlush() {
        ...
        obj.flush();
        Assert.assertTrue("Version for 'TreatmentGroup' failed to increment on flush directive", (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    ...
    public void testMergeUpdate() {
        ...
        Assert.assertTrue("Version for 'TreatmentGroup' failed to increment on merge and flush directive", (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    ...
    public void testRemove() {
        ...
        obj.remove();
        obj.flush();
        Assert.assertNull("Failed to remove 'TreatmentGroup' with identifier '" + id + "'", TreatmentGroup.findTreatmentGroup(id));
    }
}
