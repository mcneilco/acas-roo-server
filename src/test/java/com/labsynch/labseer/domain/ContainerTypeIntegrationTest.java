...
@Transactional
@RooIntegrationTest(entity = ContainerType.class)
public class ContainerTypeIntegrationTest {
    ...
    public void testCountContainerTypes() {
        Assert.assertNotNull("Data on demand for 'ContainerType' failed to initialize correctly", dod.getRandomContainerType());
    ...
    public void testFindAllContainerTypes() {
        Assert.assertNotNull("Data on demand for 'ContainerType' failed to initialize correctly", dod.getRandomContainerType());
        long count = ContainerType.countContainerTypes();
        Assert.assertTrue("Too expensive to perform a find all test for 'ContainerType', as there are " + count + " entries; set the findAllMaximum to exceed this value or set findAll=false on the integration test annotation to disable the test", count < 250);
    ...
    public void testFindContainerTypeEntries() {
        ...
        Assert.assertEquals("Find entries method for 'ContainerType' returned an incorrect number of entries", count, result.size());
    }
    ...
    public void testFlush() {
        ...
        obj.flush();
        Assert.assertTrue("Version for 'ContainerType' failed to increment on flush directive", (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    ...
    public void testMergeUpdate() {
        ...
        Assert.assertTrue("Version for 'ContainerType' failed to increment on merge and flush directive", (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    ...
    public void testRemove() {
        ...
        obj.remove();
        obj.flush();
        Assert.assertNull("Failed to remove 'ContainerType' with identifier '" + id + "'", ContainerType.findContainerType(id));
    }
}
