...
public class SubjectStateIntegrationTest {
    ...
    public void testCountSubjectStates() {
        Assert.assertNotNull("Data on demand for 'SubjectState' failed to initialize correctly",
                dod.getRandomSubjectState());
    ...
    public void testFindAllSubjectStates() {
        Assert.assertNotNull("Data on demand for 'SubjectState' failed to initialize correctly",
                dod.getRandomSubjectState());
        long count = SubjectState.countSubjectStates();
        Assert.assertTrue("Too expensive to perform a find all test for 'SubjectState', as there are " + count
    ...
    public void testFindSubjectStateEntries() {
        ...
        Assert.assertEquals("Find entries method for 'SubjectState' returned an incorrect number of entries", count,
                result.size());
    }
    ...
    public void testFlush() {
        ...
        obj.flush();
        Assert.assertTrue("Version for 'SubjectState' failed to increment on flush directive",
    ...
    public void testMergeUpdate() {
        ...
        Assert.assertEquals("Identifier of merged object not the same as identifier of original object", merged.getId(),
                id);
        Assert.assertTrue("Version for 'SubjectState' failed to increment on merge and flush directive",
                (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    ...
    public void testRemove() {
        ...
        obj.remove();
        obj.flush();
        Assert.assertNull("Failed to remove 'SubjectState' with identifier '" + id + "'",
                SubjectState.findSubjectState(id));
    }
}
