...
@Configurable
@Component
@RooDataOnDemand(entity = SubjectValue.class)
public class SubjectValueDataOnDemand {
	...
	public SubjectValue getSpecificSubjectValue(int index) {
        ...
        SubjectValue obj = data.get(index);
        Long id = obj.getId();
	...
	public void init() {
        ...
        if (data == null) {
            throw new IllegalStateException("Find entries implementation for 'SubjectValue' illegally returned null");
        }
        if (!data.isEmpty()) {
            return;
        }
