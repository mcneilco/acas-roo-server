...
@Transactional
@RooIntegrationTest(entity = ItxSubjectContainer.class)
public class ItxSubjectContainerIntegrationTest {
    ...
    public void testCountItxSubjectContainers() {
        Assert.assertNotNull("Data on demand for 'ItxSubjectContainer' failed to initialize correctly", dod.getRandomItxSubjectContainer());
    ...
    public void testFindAllItxSubjectContainers() {
        Assert.assertNotNull("Data on demand for 'ItxSubjectContainer' failed to initialize correctly", dod.getRandomItxSubjectContainer());
        long count = ItxSubjectContainer.countItxSubjectContainers();
        Assert.assertTrue("Too expensive to perform a find all test for 'ItxSubjectContainer', as there are " + count + " entries; set the findAllMaximum to exceed this value or set findAll=false on the integration test annotation to disable the test", count < 250);
    ...
    public void testFindItxSubjectContainerEntries() {
        ...
        Assert.assertEquals("Find entries method for 'ItxSubjectContainer' returned an incorrect number of entries", count, result.size());
    }
    ...
    public void testFlush() {
        ...
        obj.flush();
        Assert.assertTrue("Version for 'ItxSubjectContainer' failed to increment on flush directive", (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    ...
    public void testMergeUpdate() {
        ...
        Assert.assertTrue("Version for 'ItxSubjectContainer' failed to increment on merge and flush directive", (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    ...
    public void testRemove() {
        ...
        obj.remove();
        obj.flush();
        Assert.assertNull("Failed to remove 'ItxSubjectContainer' with identifier '" + id + "'", ItxSubjectContainer.findItxSubjectContainer(id));
    }
}
