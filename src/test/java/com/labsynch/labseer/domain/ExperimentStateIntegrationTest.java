...
public class ExperimentStateIntegrationTest {
    ...
    public void testCountExperimentStates() {
        Assert.assertNotNull("Data on demand for 'ExperimentState' failed to initialize correctly",
                dod.getRandomExperimentState());
    ...
    public void testFindAllExperimentStates() {
        Assert.assertNotNull("Data on demand for 'ExperimentState' failed to initialize correctly",
                dod.getRandomExperimentState());
        long count = ExperimentState.countExperimentStates();
        Assert.assertTrue("Too expensive to perform a find all test for 'ExperimentState', as there are " + count
    ...
    public void testFindExperimentStateEntries() {
        ...
        Assert.assertEquals("Find entries method for 'ExperimentState' returned an incorrect number of entries", count,
                result.size());
    }
    ...
    public void testFlush() {
        ...
        obj.flush();
        Assert.assertTrue("Version for 'ExperimentState' failed to increment on flush directive",
    ...
    public void testMergeUpdate() {
        ...
        Assert.assertEquals("Identifier of merged object not the same as identifier of original object", merged.getId(),
                id);
        Assert.assertTrue("Version for 'ExperimentState' failed to increment on merge and flush directive",
                (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    ...
    public void testRemove() {
        ...
        obj.remove();
        obj.flush();
        Assert.assertNull("Failed to remove 'ExperimentState' with identifier '" + id + "'",
                ExperimentState.findExperimentState(id));
    }
}
