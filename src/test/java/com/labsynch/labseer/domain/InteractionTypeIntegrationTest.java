...
public class InteractionTypeIntegrationTest {
    ...
    public void testCountInteractionTypes() {
        Assert.assertNotNull("Data on demand for 'InteractionType' failed to initialize correctly",
                dod.getRandomInteractionType());
    ...
    public void testFindAllInteractionTypes() {
        Assert.assertNotNull("Data on demand for 'InteractionType' failed to initialize correctly",
                dod.getRandomInteractionType());
        long count = InteractionType.countInteractionTypes();
        Assert.assertTrue("Too expensive to perform a find all test for 'InteractionType', as there are " + count
    ...
    public void testFindInteractionTypeEntries() {
        ...
        Assert.assertEquals("Find entries method for 'InteractionType' returned an incorrect number of entries", count,
                result.size());
    }
    ...
    public void testFlush() {
        ...
        obj.flush();
        Assert.assertTrue("Version for 'InteractionType' failed to increment on flush directive",
    ...
    public void testMergeUpdate() {
        ...
        Assert.assertEquals("Identifier of merged object not the same as identifier of original object", merged.getId(),
                id);
        Assert.assertTrue("Version for 'InteractionType' failed to increment on merge and flush directive",
                (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
    ...
    public void testRemove() {
        ...
        obj.remove();
        obj.flush();
        Assert.assertNull("Failed to remove 'InteractionType' with identifier '" + id + "'",
                InteractionType.findInteractionType(id));
    }
}
