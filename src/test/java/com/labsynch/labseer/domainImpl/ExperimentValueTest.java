...
public class ExperimentValueTest {
	...
	public void QueryExperimentValueByKinds() {
		...
		List<ExperimentValue> results = ExperimentValue.findExperimentValuesByExptIDAndStateTypeKindAndValueTypeKind(
	...
	public void QueryExperimentValueByExpIdAndStateTypeKind() {
		...
		String stateType = "metadata";
		String stateKind = "experiment metadata";
		List<ExperimentValue> results = ExperimentValue
	...
	public void QueryExperimentValueByExpIdAndStateTypeKindWithBadData() {
		...
		try {
			results = ExperimentValue.findExperimentValuesByExptIDAndStateTypeKind(experimentId, stateType, stateKind)
					.getResultList();
	...
	public void QueryExperimentValueByExpIdAndStateTypeKindWithCodeName() {
		...
		List<ExperimentValue> results = new ArrayList<ExperimentValue>();
		try {
			results = ExperimentValue
					.findExperimentValuesByExptIDAndStateTypeKind(experiment.getId(), stateType, stateKind)
					.getResultList();
		} catch (IllegalArgumentException ex) {
			logger.info(ex.getMessage());
			assert (results.size() == 0);
