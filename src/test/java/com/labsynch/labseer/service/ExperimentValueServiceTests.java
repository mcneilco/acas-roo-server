...
public class ExperimentValueServiceTests {
	...
	//
	// List<ExperimentValue> values = ExperimentValue.findExperimentValueEntries(0,
	// 3);
	...
	// Long id =
	// ApiExperimentController.retrieveExperimentIdFromCodeName("EXPT-00000003");
	// List<ExperimentValue> experimentValues =
	// experimentValueService.getExperimentValuesById(id);
	// assert(experimentValues != null);
	// assert(!experimentValues.isEmpty());
	// }
	...
	public void updateExperimentValueByIdTest() {
		...
		// ExperimentValue experimentValue2 =
		// experimentValueService.updateExperimentValue(ExperimentValue.from)
	}
	...
	public void QueryExperimentValueByKinds() {
		...
		List<ExperimentValue> results = experimentValueService
				.getExperimentValuesByExperimentIdAndStateTypeKindAndValueTypeKind(experimentId, stateType, stateKind,
						valueType, valueKind);
	...
	public void QueryExperimentValueByExpIdAndStateTypeKind() {
		...
		String stateKind = "experiment metadata";
		List<ExperimentValue> results = experimentValueService
				.getExperimentValuesByExperimentIdAndStateTypeKind(experimentId, stateType, stateKind);
		logger.info(ExperimentValue.toJsonArray(results));
	...
	public void QueryExperimentValueByExpIdAndStateTypeKindWithBadData() {
		...
		try {
			results = experimentValueService.getExperimentValuesByExperimentIdAndStateTypeKind(experimentId, stateType,
					stateKind);
	...
	public void QueryExperimentValueByExpIdAndStateTypeKindWithCodeName() {
		...
		List<ExperimentValue> results = new ArrayList<ExperimentValue>();
		try {
			results = experimentValueService.getExperimentValuesByExperimentIdAndStateTypeKind(experiment.getId(),
					stateType, stateKind);
		} catch (IllegalArgumentException ex) {
			logger.info(ex.getMessage());
			assert (results.size() == 0);
