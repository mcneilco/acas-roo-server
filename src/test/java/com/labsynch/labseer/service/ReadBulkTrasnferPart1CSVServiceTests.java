...
public class ReadBulkTrasnferPart1CSVServiceTests {
	...
	public void ReadCSVFile_Test1() throws IOException {
		...
		// ICsvMapWriter writer = new CsvMapWriter(csvFileOut,
		...
		// BulkTransferPart1DTO ick = beanReader.read(BulkTransferPart1DTO.class,
		// header, processors);
		...
		try {
			...
			final String[] header = new String[] { "sourceBarcode", "sourceWell", "destinationBarcode",
					"destinationPlateSize", "destinationWell",
					"amountTransferred", "amountUnits", "finalConcentration", "concUnits", "finalVolume",
					"volumeUnits", "liquidType", "dateTime", "transferProtocol", "isNewPlate",
					"possibleTransferError" };
			...
			while ((bulkData = beanReader.read(BulkTransferPart1DTO.class, header, processors)) != null) {
				System.out.println(String.format("lineNo=%s, rowNo=%s, bulkData=%s", beanReader.getLineNumber(),
						beanReader.getRowNumber(), bulkData));

				Container destinationPlate = getPlate(bulkData.getDestinationBarcode(),
						bulkData.getDestinationPlateSize());
		...
		// ICsvMapReader inputMapFile = new CsvMapReader(new StringReader(csvFile),
		// CsvPreference.EXCEL_PREFERENCE);
		Map<String, String> dataMapIn = new HashMap<String, String>();
		StringWriter outFile = new StringWriter();
		// ICsvMapWriter writer = new CsvMapWriter(outFile,
		// CsvPreference.EXCEL_PREFERENCE);
		try {
	...
	private Container getPlate(String destinationBarcode,
		...
		if (foundContainer.size() == 0) {
			...
			logger.warn(
					"found the container" + destinationBarcode + "  number of containers: " + foundContainer.size());
	...
	private static CellProcessor[] getProcessors() {
		...
		// final String[] header = new String[] { "sourceBarcode", "sourceWell",
		// "destinationBarcode", "destinationWell",
		// "destinationPlateSize", "amountTransferred", "amountUnits",
		// "finalConcentration", "concUnits", "finalVolume",
		// "volumeUnits", "liquidType", "dateTime", "transferProtocol", "isNewPlate",
		// "possibleTransferError"};
