...
//		TreatmentGroupValue findTgv = TreatmentGroupValue.findTreatmentGroupValue(tgv.getId());
...
//		logger.info(Experiment.findExperiment(experiment.getId()).toJson());
	...
	public void UpdateExeprimentName_2(){
		...
		experimentService.updateExperiment(Experiment.fromJsonToExperiment(json));		
	}
	
	//@Test
	//@Transactional
	public void CreateProtocolFromSimpleJson_3() throws UniqueExperimentNameException{
		...
		Experiment experiment = experimentService.saveLsExperiment(Experiment.fromJsonToExperiment(json));		
		logger.info(experiment.toJson());
	...
	//@Test
	//@Transactional
	public void CreateProtocolFromNestedJson() throws UniqueExperimentNameException{
...
//			List<ItxSubjectContainer> results = ItxSubjectContainer.findItxSubjectContainerByExperimentID(experiment.getId());
//			logger.debug("number of results: " + results.size());
			int dels = ItxSubjectContainer.deleteByExperimentID(experiment.getId());
...
//		
////		logger.debug("deleting analysis groups within an experiment: " + id);
////		
...
//		logger.debug("total number of numberOfAnalysisGroups: " + numberOfAnalysisGroups);
//
//		HashSet<Long> lsTranasactionIds = new HashSet<Long>();
//		for (AnalysisGroup analysisGroup : analysisGroups){
//			lsTranasactionIds.add(analysisGroup.getLsTransaction());
//	}
//	logger.debug("total number of lsTranasactionIds: " + lsTranasactionIds.size());
		
		
		//
//		Collection<TreatmentGroup> treamentGroupSet = new HashSet<TreatmentGroup>();
//		for (AnalysisGroup analysisGroup : analysisGroups){
//			List<TreatmentGroup> treatmentGroups = TreatmentGroup.findTreatmentGroupsByAnalysisGroup(analysisGroup).getResultList();
//			logger.debug("found number of treatment group per analyisGroup: " + analysisGroup.getId() + "  " + treatmentGroups.size());
//			treamentGroupSet.addAll(treatmentGroups);
//		}
//		logger.debug("total number of treamentGroups: " + treamentGroupSet.size());
//		
//		Collection<Subject> subjectSet = new HashSet<Subject>();
//		for (TreatmentGroup treamentGroup : treamentGroupSet){
//			List<Subject> subjectGroups = Subject.findSubjectsByTreatmentGroup(treamentGroup).getResultList();
//			logger.debug("found number of subject group per treamentGroup: " + treamentGroup.getId() + "  " + subjectGroups.size());
...
//			//int deletedStates = SubjectState.deleteByTransactionID(transactionId);
...
//		}
		...
		logger.debug("deleted the experiment by cascade!"); //takes about a minute

//		
//		int deletedAnalysisGroups = AnalysisGroup.deleteByExperimentID(id);
//		logger.debug("number of deletedAnalysisGroups: " + deletedAnalysisGroups);
	}
	...
	public void findExperiementByNameAndProtocol(){
		...
		List<ExperimentLabel> labels = ExperimentLabel.findExperimentLabelsByNameAndProtocol(labelText, labelType, labelKind, preferred, ignored, protocolId).getResultList();
