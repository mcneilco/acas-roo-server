...
public class RegisterCompoundsController {
	...
	public ResponseEntity<String> searchCmpdsByParamsPost(
		...
		if (compoundAlias != null)
			propertiesMap.put("compound_alias", compoundAlias);
		if (supplierId != null)
			propertiesMap.put("supplier_id", supplierId);
		if (project != null)
			propertiesMap.put("project", project);
		if (supplier != null)
			propertiesMap.put("supplier", supplier);
		if (notebook != null)
			propertiesMap.put("notebook", notebook);
		if (stereoCategory != null)
			propertiesMap.put("stereo_category", stereoCategory);
		if (compoundChemist != null)
			propertiesMap.put("compound_chemist", compoundChemist);
		if (compoundChemistCode != null)
			propertiesMap.put("compound_chemist_code", compoundChemistCode);
		...
		// 'http://localhost:8080/cmpdreg/api/v1/registration/cmpds?inputFileName=/tmp/spark1.sdf&errorFileName=/tmp/spark1_errorMols.sdf&compoundAlias=ALIAS&supplierId=VENDORCATNO&project=PROJECT&supplier=IGNYTA&notebook=NOTEBOOK&stereoCategory=unknown&compoundChemist=CHEMIST&compoundChemistCode=chemadmin'
	}
