...
public class ApiStructureServicesController {
	...
	public ResponseEntity<String> standardize(@RequestBody String json) {
		...
		try {
			...
			logger.error(e.toString());
			return new ResponseEntity<String>("Cannot read input molfile: " + e.toString(), headers,
					HttpStatus.BAD_REQUEST);
		} catch (StandardizerException e) {
			logger.error(e.toString());
			return new ResponseEntity<String>("Standardization ERROR: " + e.toString(), headers,
					HttpStatus.BAD_REQUEST);
		}
	...
	public ResponseEntity<String> molconvert(@RequestBody String json) {
		...
		try {
			output = chemStructureService.toFormat(inputDTO.getStructure(), inputDTO.getInputFormat(),
					inputDTO.getParameters());
		} catch (IOException e) {
			logger.error(e.toString());
			return new ResponseEntity<String>("IO ERROR", headers, HttpStatus.BAD_REQUEST);
		} catch (CmpdRegMolFormatException e) {
	...
	public ResponseEntity<String> cleanMol(@RequestBody String json) {
		if (logger.isDebugEnabled())
			logger.debug("incoming json from mol clean: " + json);
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "plain/text");
		MolCleanInputDTO inputDTO = MolCleanInputDTO.fromJsonToMolCleanInputDTO(json);
		MolConvertOutputDTO output = null;
		try {
			output = chemStructureService.cleanStructure(inputDTO.getStructure(), inputDTO.getParameters().getDim(),
					inputDTO.getParameters().getOpts());
		} catch (IOException e) {
			logger.error(e.toString());
			return new ResponseEntity<String>("IO ERROR", headers, HttpStatus.BAD_REQUEST);
		} catch (CmpdRegMolFormatException e) {
	...
	public ResponseEntity<String> hydrogenizeMol(@RequestBody String json) {
		if (logger.isDebugEnabled())
			logger.debug("incoming json from mol hydrogenize: " + json);
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "plain/text");
		HydrogenizeMolInputDTO inputDTO = HydrogenizeMolInputDTO.fromJsonToHydrogenizeMolInputDTO(json);
		String output = null;
		try {
			output = chemStructureService.hydrogenizeMol(inputDTO.getStructure(), inputDTO.getInputFormat(),
					inputDTO.getParameters().getMethod());
		} catch (IOException e) {
			logger.error(e.toString());
			return new ResponseEntity<String>("IO ERROR", headers, HttpStatus.BAD_REQUEST);
		} catch (CmpdRegMolFormatException e) {
	...
	public ResponseEntity<String> cipStereoInfo(@RequestBody String json) {
		if (logger.isDebugEnabled())
			logger.debug("incoming json from mol get cipStereoInfo: " + json);
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "plain/text");
