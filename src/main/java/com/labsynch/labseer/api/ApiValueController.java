...
public class ApiValueController {
	...
	public ResponseEntity<String> putValueByPath(
		...
		if (entity.equals("protocol")) {
			ProtocolValue protocolValue = protocolValueService.updateProtocolValue(idOrCodeName, stateType, stateKind,
		...
		if (entity.equals("experiment")) {
			ExperimentValue experimentValue = experimentValueService.updateExperimentValue(idOrCodeName, stateType,
					stateKind, valueType, valueKind, value);
			return new ResponseEntity<String>(experimentValue.toJson(), headers, HttpStatus.OK);
		}
		if (entity.equals("analysisGroup")) {
			AnalysisGroupValue analysisGroupValue = analysisGroupValueService.updateAnalysisGroupValue(idOrCodeName,
					stateType, stateKind, valueType, valueKind, value);
			return new ResponseEntity<String>(analysisGroupValue.toJson(), headers, HttpStatus.OK);
		}
		if (entity.equals("treatmentGroup")) {
			TreatmentGroupValue treatmentGroupValue = treatmentGroupValueService.updateTreatmentGroupValue(idOrCodeName,
					stateType, stateKind, valueType, valueKind, value);
			return new ResponseEntity<String>(treatmentGroupValue.toJson(), headers, HttpStatus.OK);
		}
		if (entity.equals("subject")) {
			SubjectValue subjectValue = subjectValueService.updateSubjectValue(idOrCodeName, stateType, stateKind,
					valueType, valueKind, value);
		...
		if (entity.equals("container")) {
			ContainerValue containerValue = containerValueService.updateContainerValue(idOrCodeName, stateType,
					stateKind, valueType, valueKind, value);
	...
	public ResponseEntity<String> getValueByPathWithCodeNameArray(
		...
		if (entity.equals("protocol")) {
			...
			for (String codeName : codeNames) {
				ProtocolValue protocolValue = protocolValueService.getProtocolValue(codeName, stateType, stateKind,
						valueType, valueKind);
			...
			if (values.isEmpty())
				...
				return new ResponseEntity<String>(CodeNameValueDTO.toJsonArray(values), headers, HttpStatus.OK);
		}
		if (entity.equals("experiment")) {
			Collection<CodeNameValueDTO> values = new ArrayList<CodeNameValueDTO>();
			for (String codeName : codeNames) {
				ExperimentValue experimentValue = experimentValueService.getExperimentValue(codeName, stateType,
		...
		if (entity.equals("analysisGroup")) {
			Collection<CodeNameValueDTO> values = new ArrayList<CodeNameValueDTO>();
			for (String codeName : codeNames) {
				AnalysisGroupValue analysisGroupValue = analysisGroupValueService.getAnalysisGroupValue(codeName,
		...
		if (entity.equals("treatmentGroup")) {
			Collection<CodeNameValueDTO> values = new ArrayList<CodeNameValueDTO>();
			for (String codeName : codeNames) {
				TreatmentGroupValue treatmentGroupValue = treatmentGroupValueService.getTreatmentGroupValue(codeName,
						stateType, stateKind, valueType, valueKind);
		...
		if (entity.equals("subject")) {
			Collection<CodeNameValueDTO> values = new ArrayList<CodeNameValueDTO>();
			for (String codeName : codeNames) {
				SubjectValue subjectValue = subjectValueService.getSubjectValue(codeName, stateType, stateKind,
						valueType, valueKind);
		...
		if (entity.equals("lsThing")) {
			Collection<CodeNameValueDTO> values = new ArrayList<CodeNameValueDTO>();
			for (String codeName : codeNames) {
				LsThingValue lsThingValue = lsThingValueService.getLsThingValue(codeName, stateType, stateKind,
						valueType, valueKind);
	...
	public ResponseEntity<String> getValueByPath(
		...
		logger.debug("ENTITY IS: " + entity);
		if (entity.equals("protocol")) {
			ProtocolValue protocolValue = protocolValueService.getProtocolValue(idOrCodeName, stateType, stateKind,
					valueType, valueKind);
		...
		if (entity.equals("experiment")) {
			ExperimentValue experimentValue = experimentValueService.getExperimentValue(idOrCodeName, stateType,
					stateKind, valueType, valueKind);
		...
		if (entity.equals("analysisGroup")) {
			AnalysisGroupValue analysisGroupValue = analysisGroupValueService.getAnalysisGroupValue(idOrCodeName,
					stateType, stateKind, valueType, valueKind);
		...
		if (entity.equals("treatmentGroup")) {
			TreatmentGroupValue treatmentGroupValue = treatmentGroupValueService.getTreatmentGroupValue(idOrCodeName,
					stateType, stateKind, valueType, valueKind);
		...
		if (entity.equals("subject")) {
			SubjectValue subjectValue = subjectValueService.getSubjectValue(idOrCodeName, stateType, stateKind,
					valueType, valueKind);
		...
		if (entity.equals("lsThing")) {
			LsThingValue lsThingValue = lsThingValueService.getLsThingValue(idOrCodeName, stateType, stateKind,
					valueType, valueKind);
		...
		if (entity.equals("container")) {
			ContainerValue containerValue = containerValueService.getContainerValue(idOrCodeName, stateType, stateKind,
					valueType, valueKind);
	...
	public ResponseEntity<String> getValuesByPaths(
		...
		Collection<GenericValuePathRequest> genericRequests = GenericValuePathRequest
				.fromJsonArrayToGenericValuePathRequests(json);
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
		// this if/else if block controls which lsThing is being hit
		logger.debug("ENTITY IS: " + entity);
		...
		if (entity.equals("analysisGroup")) {
			Collection<AnalysisGroupValuePathDTO> results = analysisGroupValueService
					.getAnalysisGroupValues(genericRequests);
			return new ResponseEntity<String>(AnalysisGroupValuePathDTO.toJsonArray(results), headers, HttpStatus.OK);
		}
		if (entity.equals("treatmentGroup")) {
			Collection<TreatmentGroupValuePathDTO> results = treatmentGroupValueService
					.getTreatmentGroupValues(genericRequests);
	...
	public ResponseEntity<String> listExperimentValuesJsonArray() {
		...
		for (ExperimentValue experimentValue : result) {
			if (!experimentValue.isIgnored())
				finalResult.add(experimentValue);
		}
	...
	public ResponseEntity<String> listAnalysisGroupValuesJsonArray() {
		...
		for (AnalysisGroupValue analysisGroupValue : result) {
			if (!analysisGroupValue.isIgnored())
				finalResult.add(analysisGroupValue);
		}
	...
	public ResponseEntity<String> listTreatmentGroupValuesJsonArray() {
		...
		for (TreatmentGroupValue treatmentGroupValue : result) {
			if (!treatmentGroupValue.isIgnored())
				finalResult.add(treatmentGroupValue);
		}
	...
	public ResponseEntity<String> listSubjectValuesJsonArray() {
		...
		for (SubjectValue subjectValue : result) {
			if (!subjectValue.isIgnored())
				finalResult.add(subjectValue);
		}
	...
	public ResponseEntity<String> listLsThingValuesJsonArray() {
		...
		for (LsThingValue lsThingValue : result) {
			if (!lsThingValue.isIgnored())
				finalResult.add(lsThingValue);
		}
	...
	public ResponseEntity<String> listContainerValuesJsonArray() {
		...
		for (ContainerValue containerValue : result) {
			if (!containerValue.isIgnored())
				finalResult.add(containerValue);
		}
	...
	public ResponseEntity<String> showContainerValueJson(@PathVariable("id") Long id) {
		...
		if (containerValue == null || containerValue.isIgnored())
			return new ResponseEntity<String>(headers, HttpStatus.NOT_FOUND);
		return new ResponseEntity<String>(containerValue.toJson(), headers, HttpStatus.OK);
	}
	...
	@ResponseBody
	@Transactional
	public ResponseEntity<String> updateTreatmentGroupValueFromJson(
			@RequestBody TreatmentGroupValue treatmentGroupValue) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
	...
	@ResponseBody
	@Transactional
	public ResponseEntity<String> updateExperimentValuesFromJsonArray(
			@RequestBody List<ExperimentValue> experimentValues) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
	...
	@ResponseBody
	@Transactional
	public ResponseEntity<String> updateAnalysisGroupValuesFromJsonArray(
			@RequestBody List<AnalysisGroupValue> analysisGroupValues) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
		analysisGroupValues = (List<AnalysisGroupValue>) analysisGroupValueService
				.updateAnalysisGroupValues(analysisGroupValues);
	...
	@ResponseBody
	@Transactional
	public ResponseEntity<String> updateTreatmentGroupValuesFromJsonArray(
			@RequestBody List<TreatmentGroupValue> treatmentGroupValues) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
		treatmentGroupValues = (List<TreatmentGroupValue>) treatmentGroupValueService
				.updateTreatmentGroupValues(treatmentGroupValues);
	...
	@ResponseBody
	@Transactional
	public ResponseEntity<String> updateContainerValuesFromJsonArray(
			@RequestBody List<ContainerValue> containerValues) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
	...
	@ResponseBody
	@Transactional
	public ResponseEntity<String> createTreatmentGroupValueFromJson(
			@RequestBody TreatmentGroupValue treatmentGroupValue) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
	...
	@ResponseBody
	@Transactional
	public ResponseEntity<String> createExperimentValuesFromJsonArray(
			@RequestBody List<ExperimentValue> experimentValues) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
	...
	@ResponseBody
	@Transactional
	public ResponseEntity<String> createAnalysisGroupValuesFromJsonArray(
			@RequestBody List<AnalysisGroupValue> analysisGroupValues) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
		analysisGroupValues = (List<AnalysisGroupValue>) analysisGroupValueService
				.saveAnalysisGroupValues(analysisGroupValues);
		return new ResponseEntity<String>(AnalysisGroupValue.toJsonArray(analysisGroupValues), headers, HttpStatus.OK);
	}
	...
	@ResponseBody
	@Transactional
	public ResponseEntity<String> createTreatmentGroupValuesFromJsonArray(
			@RequestBody List<TreatmentGroupValue> treatmentGroupValues) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
		treatmentGroupValues = (List<TreatmentGroupValue>) treatmentGroupValueService
				.saveTreatmentGroupValues(treatmentGroupValues);
		return new ResponseEntity<String>(TreatmentGroupValue.toJsonArray(treatmentGroupValues), headers,
				HttpStatus.OK);
	}
	...
	@ResponseBody
	@Transactional
	public ResponseEntity<String> createContainerValuesFromJsonArray(
			@RequestBody List<ContainerValue> containerValues) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
	...
	public ResponseEntity<String> createLsThingValuesFromJsonArray(@RequestParam("codeValue") String codeValue) {
		...
		headers.add("Content-Type", "application/json; charset=utf-8");
		try {
			Collection<LsThingValue> lsThingValues = LsThingValue.findLsThingValuesByCodeValueEquals(codeValue)
					.getResultList();
			return new ResponseEntity<String>(LsThingValue.toJsonArray(lsThingValues), headers, HttpStatus.OK);
		} catch (Exception e) {
