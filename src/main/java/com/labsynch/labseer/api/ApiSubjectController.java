...
public class ApiSubjectController {
	...
	// @RequestMapping(value = "/{SubjectIdOrCodeName}/values/{SubjectValueId}",
	...
	public ResponseEntity<String> getSubjectValuesForSubjectByIdOrCodeName(
		...
		Long subjectId = SimpleUtil.isNumeric(subjectIdOrCodeName) ? Long.valueOf(subjectIdOrCodeName)
				: retrieveSubjectIdFromCodeName(subjectIdOrCodeName);
		if (subjectId != null) {
			subjectValues = subjectValueService.getSubjectValuesBySubjectId(subjectId);
		}

		return (subjectValues == null) ? new ResponseEntity<String>(headers, HttpStatus.NOT_FOUND)
	...
	private static Long retrieveSubjectIdFromCodeName(String codeName) {
		...
		return (subjects == null || subjects.size() == 0) ? null : subjects.get(0).getId();
	}
	...
	// @RequestMapping(value = "{SubjectIdOrCodeName}/values", method =
	...
	public ResponseEntity<String> jsonFindSubjectsByCodeNameEquals(@RequestParam("codeName") String codeName) {
		...
		return new ResponseEntity<String>(
				Subject.toJsonArray(Subject.findSubjectsByCodeNameEquals(codeName).getResultList()), headers,
				HttpStatus.OK);
	}

	@RequestMapping(params = "find=ByLsTransactionEquals", method = RequestMethod.GET, headers = "Accept=application/json")
	@ResponseBody
	public ResponseEntity<String> jsonFindSubjectsByLsTransactionEquals(
			@RequestParam("lsTransaction") Long lsTransaction) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
		return new ResponseEntity<String>(
				Subject.toJsonArray(Subject.findSubjectsByLsTransactionEquals(lsTransaction).getResultList()), headers,
				HttpStatus.OK);
	}

	// @RequestMapping(params = "find=ByTreatmentGroup", headers =
	// "Accept=application/json")
	// @ResponseBody
	// public ResponseEntity<String>
	// jsonFindSubjectsByTreatmentGroup(@RequestParam("treatmentGroup")
	// TreatmentGroup treatmentGroup) {
	// HttpHeaders headers = new HttpHeaders();
	// headers.add("Content-Type", "application/json; charset=utf-8");
	// return new
	// ResponseEntity<String>(Subject.toJsonArray(Subject.findSubjectsByTreatmentGroups(treatmentGroup).getResultList()),
	...
	public @ResponseBody ResponseEntity<String> saveSubjectsFromCsv(@RequestBody SubjectCsvDataDTO subjectCsvDataDTO) {
		...
		try {
			...
			HashMap<String, TempThingDTO> resultMap = subjectService.createOnlySubjectsFromCSV(subjectFilePath,
					subjectCsvDataDTO.getTreatmentGroupIds());
	...
	public ResponseEntity<java.lang.String> getSubjectsByContainerInteractions(@RequestBody String json,
		...
		try {
			...
			if (with != null && with.equalsIgnoreCase("stub")) {
				return new ResponseEntity<String>(ContainerSubjectsDTO.toJsonArrayStub(results), headers,
						HttpStatus.OK);
			}
	...
	public ResponseEntity<java.lang.String> searchSubjects(@RequestBody String json,
			@RequestParam(value = "with", required = false) String with) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
	...
	public ResponseEntity<java.lang.String> searchSubjectsWithContainerCodeArray(@RequestBody String json,
			@RequestParam(value = "with", required = false) String with) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
		MultiContainerSubjectSearchRequest query = MultiContainerSubjectSearchRequest
				.fromJsonToMultiContainerSubjectSearchRequest(json);
		ArrayList<ErrorMessage> errors = new ArrayList<ErrorMessage>();
		boolean errorsFound = false;
