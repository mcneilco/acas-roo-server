...
public class ApiStructureController {
	...
	public ResponseEntity<byte[]> renderMolStructure(@RequestBody String json) {
		...
		try {
			byte[] image = structureService.renderMolStructure(request.getMolStructure(), request.getHeight(),
					request.getWidth(), request.getFormat());
	...
	public ResponseEntity<String> renderMolStructureBase64(@RequestBody String json) {
		...
		logger.info("incoming request json: " + request.toJson());
		try {
			String image = structureService.renderMolStructureBase64(request.getMolStructure(), request.getHeight(),
					request.getWidth(), request.getFormat());
	...
	@ResponseBody
	public ResponseEntity<byte[]> renderStructureByCodeName(
	...
	@ResponseBody
	public ResponseEntity<byte[]> renderStructureByLsThingCodeName(
			@RequestParam(value = "codeName", required = true) String codeName,
			@RequestParam(value = "height", required = true) int height,
			@RequestParam(value = "width", required = true) int width,
			@RequestParam(value = "format", required = true) String format) {
	...
	public ResponseEntity<java.lang.String> searchStructures(@RequestBody String json) {
		...
		try {
			...
			if (query.getLsType() != null && query.getLsKind() != null) {
				structures = structureService.searchStructuresByTypeKind(query.getQueryMol(), query.getLsType(),
				...
				structures = structureService.searchStructures(query.getQueryMol(),
						SearchType.getIfPresent(query.getSearchType()).orElse(SearchType.DEFAULT),
						query.getMaxResults(), query.getSimilarity());
			}
