...
public class SubjectValueServiceImpl implements SubjectValueService {
	...
	public List<SubjectValue> getSubjectValuesByExperimentIdAndStateTypeKindAndValueTypeKind(
		...
		List<SubjectValue> subjectValues = SubjectValue
				.findSubjectValuesByExptIDAndStateTypeKindAndValueTypeKind(experimentId, stateType,
	...
	public List<SubjectValue> getSubjectValuesBySubjectIdAndStateTypeKindAndValueTypeKind(
		...
		List<SubjectValue> subjectValues = SubjectValue
				.findSubjectValuesBySubjectIDAndStateTypeKindAndValueTypeKind(subjectId, stateType,
						stateKind, valueType, valueKind)
				.getResultList();
	...
	@Override
	public SubjectValue updateSubjectValue(String idOrCodeName, String stateType, String stateKind, String valueType,
		...
		if (subject != null) {
			...
			subjectStates = subjectStateService.getSubjectStatesBySubjectIdAndStateTypeKind(subjectId, stateType,
					stateKind);
			if (subjectStates.isEmpty()) {
				// create the state
				subjectStates.add(subjectStateService.createSubjectStateBySubjectIdAndStateTypeKind(subjectId,
						stateType, stateKind));
		...
		if (subject != null) {
			Long subjectId = subject.getId();
			subjectValues = getSubjectValuesBySubjectIdAndStateTypeKindAndValueTypeKind(subjectId, stateType, stateKind,
					valueType, valueKind);
			...
			subjectValue = createSubjectValueBySubjectIdAndStateTypeKindAndValueTypeKind(subjectId, stateType,
	...
	private SubjectValue createSubjectValueBySubjectIdAndStateTypeKindAndValueTypeKind(
		...
		SubjectState subjectState = SubjectState
				.findSubjectStatesBySubjectIDAndStateTypeKind(subjectId, stateType, stateKind).getSingleResult();
		subjectValue.setLsState(subjectState);
		...
		if (valueType.equals("fileValue"))
			subjectValue.setFileValue(value);
		if (valueType.equals("clobValue"))
			subjectValue.setClobValue(value);
		if (valueType.equals("blobValue"))
			subjectValue.setBlobValue(value.getBytes(Charset.forName("UTF-8")));
		if (valueType.equals("numericValue"))
			subjectValue.setNumericValue(new BigDecimal(value));
		if (valueType.equals("dateValue"))
			subjectValue.setDateValue(new Date(Long.parseLong(value)));
		if (valueType.equals("codeValue"))
			subjectValue.setCodeValue(value);
		if (valueType.equals("urlValue"))
			subjectValue.setUrlValue(value);
	...
	public SubjectValue getSubjectValue(String idOrCodeName,
		...
		try {
			Collection<SubjectValue> values = getSubjectValues(idOrCodeName, stateType, stateKind, valueType,
					valueKind);
	...
	public Collection<SubjectValuePathDTO> getSubjectValues(
		...
		for (GenericValuePathRequest request : genericRequests) {
			...
			result.setValues(getSubjectValues(request.getIdOrCodeName(), request.getStateType(), request.getStateKind(),
					request.getValueType(), request.getValueKind()));
			results.add(result);
		}
		return results;
	}

	private Collection<SubjectValue> getSubjectValues(String idOrCodeName, String stateType, String stateKind,
			String valueType, String valueKind) {
		if (SimpleUtil.isNumeric(idOrCodeName)) {
			Long id = Long.valueOf(idOrCodeName);
			return SubjectValue.findSubjectValuesBySubjectIDAndStateTypeKindAndValueTypeKind(id, stateType, stateKind,
					valueType, valueKind).getResultList();
		} else {
			return SubjectValue.findSubjectValuesBySubjectCodeNameAndStateTypeKindAndValueTypeKind(idOrCodeName,
