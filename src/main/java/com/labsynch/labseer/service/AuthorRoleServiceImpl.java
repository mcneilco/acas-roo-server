...
public class AuthorRoleServiceImpl implements AuthorRoleService {
	...
	public void saveAuthorRoleDTOs(Collection<AuthorRoleDTO> authorRoleDTOs) {
		for (AuthorRoleDTO authorRoleDTO : authorRoleDTOs) {
			...
			LsRole role = LsRole.findLsRolesByLsTypeEqualsAndLsKindEqualsAndRoleNameEquals(authorRoleDTO.getRoleType(),
					authorRoleDTO.getRoleKind(), authorRoleDTO.getRoleName()).getSingleResult();
			Collection<AuthorRole> foundAuthorRoles = AuthorRole.findAuthorRolesByRoleEntryAndUserEntry(role, author)
					.getResultList();
