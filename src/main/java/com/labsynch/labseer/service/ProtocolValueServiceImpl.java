...
public class ProtocolValueServiceImpl implements ProtocolValueService {
	...
	public List<ProtocolValue> getProtocolValuesByProtocolIdAndStateTypeKindAndValueTypeKind(
		...
		List<ProtocolValue> protocolValues = ProtocolValue
				.findProtocolValuesByProtocolIDAndStateTypeKindAndValueTypeKind(protocolId, stateType,
	...
	@Override
	public ProtocolValue updateProtocolValue(String idOrCodeName, String stateType, String stateKind, String valueType,
		...
		if (protocol != null) {
			...
			protocolStates = protocolStateService.getProtocolStatesByProtocolIdAndStateTypeKind(protocolId, stateType,
					stateKind);
			if (protocolStates.isEmpty()) {
				// create the state
				protocolStates.add(protocolStateService.createProtocolStateByProtocolIdAndStateTypeKind(protocolId,
						stateType, stateKind));
		...
		if (protocol != null) {
			Long protocolId = protocol.getId();
			protocolValues = getProtocolValuesByProtocolIdAndStateTypeKindAndValueTypeKind(protocolId, stateType,
					stateKind, valueType, valueKind);
			...
			protocolValue = createProtocolValueByProtocolIdAndStateTypeKindAndValueTypeKind(protocolId, stateType,
	...
	private ProtocolValue createProtocolValueByProtocolIdAndStateTypeKindAndValueTypeKind(
		...
		ProtocolState protocolState = ProtocolState
				.findProtocolStatesByProtocolIDAndStateTypeKind(protocolId, stateType, stateKind).getSingleResult();
		protocolValue.setLsState(protocolState);
		...
		if (valueType.equals("fileValue"))
			protocolValue.setFileValue(value);
		if (valueType.equals("clobValue"))
			protocolValue.setClobValue(value);
		if (valueType.equals("blobValue"))
			protocolValue.setBlobValue(value.getBytes(Charset.forName("UTF-8")));
		if (valueType.equals("numericValue"))
			protocolValue.setNumericValue(new BigDecimal(value));
		if (valueType.equals("dateValue"))
			protocolValue.setDateValue(new Date(Long.parseLong(value)));
		if (valueType.equals("codeValue"))
			protocolValue.setCodeValue(value);
		if (valueType.equals("urlValue"))
			protocolValue.setUrlValue(value);
	...
	public ProtocolValue getProtocolValue(String idOrCodeName,
		...
		try {
			Collection<ProtocolValue> values = getProtocolValues(idOrCodeName, stateType, stateKind, valueType,
					valueKind);
	...
	public Collection<ProtocolValuePathDTO> getProtocolValues(
		...
		for (GenericValuePathRequest request : genericRequests) {
			...
			result.setValues(getProtocolValues(request.getIdOrCodeName(), request.getStateType(),
	...
	private Collection<ProtocolValue> getProtocolValues(String idOrCodeName, String stateType, String stateKind,
			String valueType, String valueKind) {
		if (SimpleUtil.isNumeric(idOrCodeName)) {
			Long id = Long.valueOf(idOrCodeName);
			return ProtocolValue.findProtocolValuesByProtocolIDAndStateTypeKindAndValueTypeKind(id, stateType,
					stateKind, valueType, valueKind).getResultList();
		} else {
			return ProtocolValue.findProtocolValuesByProtocolCodeNameAndStateTypeKindAndValueTypeKind(idOrCodeName,
