...
public class LsThingValueServiceImpl implements LsThingValueService {
	...
	public List<LsThingValue> getLsThingValuesByLsThingIdAndStateTypeKindAndValueTypeKind(
		...
		List<LsThingValue> lsThingValues = LsThingValue
				.findLsThingValuesByLsThingIDAndStateTypeKindAndValueTypeKind(lsThingId, stateType,
	...
	@Override
	public LsThingValue updateLsThingValue(String idOrCodeName, String stateType, String stateKind, String valueType,
		...
		if (lsThing != null) {
			...
			lsThingStates = lsThingStateService.getLsThingStatesByLsThingIdAndStateTypeKind(lsThingId, stateType,
					stateKind);
			if (lsThingStates.isEmpty()) {
				// create the state
				lsThingStates.add(lsThingStateService.createLsThingStateByLsThingIdAndStateTypeKind(lsThingId,
						stateType, stateKind));
		...
		if (lsThing != null) {
			Long lsThingId = lsThing.getId();
			lsThingValues = getLsThingValuesByLsThingIdAndStateTypeKindAndValueTypeKind(lsThingId, stateType, stateKind,
					valueType, valueKind);
			...
			lsThingValue = createLsThingValueByLsThingIdAndStateTypeKindAndValueTypeKind(lsThingId, stateType,
	...
	private LsThingValue createLsThingValueByLsThingIdAndStateTypeKindAndValueTypeKind(
		...
		LsThingState lsThingState = LsThingState
				.findLsThingStatesByLsThingIDAndStateTypeKind(lsThingId, stateType, stateKind).getSingleResult();
		lsThingValue.setLsState(lsThingState);
		...
		if (valueType.equals("fileValue"))
			lsThingValue.setFileValue(value);
		if (valueType.equals("clobValue"))
			lsThingValue.setClobValue(value);
		if (valueType.equals("blobValue"))
			lsThingValue.setBlobValue(value.getBytes(Charset.forName("UTF-8")));
		if (valueType.equals("numericValue"))
			lsThingValue.setNumericValue(new BigDecimal(value));
		if (valueType.equals("dateValue"))
			lsThingValue.setDateValue(new Date(Long.parseLong(value)));
		if (valueType.equals("codeValue"))
			lsThingValue.setCodeValue(value);
		if (valueType.equals("urlValue"))
			lsThingValue.setUrlValue(value);
	...
	public LsThingValue getLsThingValue(String idOrCodeName,
		...
		try {
			Collection<LsThingValue> values = getLsThingValues(idOrCodeName, stateType, stateKind, valueType,
					valueKind);
	...
	public Collection<LsThingValuePathDTO> getLsThingValues(
		...
		for (GenericValuePathRequest request : genericRequests) {
			...
			result.setValues(getLsThingValues(request.getIdOrCodeName(), request.getStateType(), request.getStateKind(),
	...
	private Collection<LsThingValue> getLsThingValues(String idOrCodeName, String stateType, String stateKind,
			String valueType, String valueKind) {
		if (SimpleUtil.isNumeric(idOrCodeName)) {
			Long id = Long.valueOf(idOrCodeName);
			return LsThingValue.findLsThingValuesByLsThingIDAndStateTypeKindAndValueTypeKind(id, stateType, stateKind,
					valueType, valueKind).getResultList();
		} else {
			return LsThingValue.findLsThingValuesByLsThingCodeNameAndStateTypeKindAndValueTypeKind(idOrCodeName,
