...
public class ExperimentValueServiceImpl implements ExperimentValueService {
	...
	// Query hibernate object and grab existing table references - add them to json
	// hydrated object
	@Override
	@Transactional
	public ExperimentValue updateExperimentValue(ExperimentValue experimentValue) {
		if (experimentValue.getLsState().getId() == null) {
			...
			experimentState
					.setExperiment(Experiment.findExperiment(experimentValue.getLsState().getExperiment().getId()));
	...
	@Override
	public List<ExperimentValue> getExperimentValuesByExperimentIdAndStateTypeKindAndValueTypeKind(Long experimentId,
			String stateType,
			String stateKind, String valueType, String valueKind) {

		List<ExperimentValue> experimentValues = ExperimentValue
				.findExperimentValuesByExptIDAndStateTypeKindAndValueTypeKind(experimentId, stateType, stateKind,
						valueType, valueKind)
				.getResultList();
	...
	public List<ExperimentValue> getExperimentValuesByExperimentIdAndStateTypeKind(Long experimentId, String stateType,
		...
		List<ExperimentValue> experimentValues = ExperimentValue
				.findExperimentValuesByExptIDAndStateTypeKind(experimentId, stateType, stateKind).getResultList();
	...
	@Override
	public ExperimentValue updateExperimentValue(String idOrCodeName, String stateType, String stateKind,
		...
		if (experiment != null) {
			...
			if (experimentStates.isEmpty()) {
				// create the state
				experimentStates.add(experimentStateService
						.createExperimentStateByExperimentIdAndStateTypeKind(experimentId, stateType, stateKind));
		...
		if (experiment != null) {
			Long experimentId = experiment.getId();
			experimentValues = getExperimentValuesByExperimentIdAndStateTypeKindAndValueTypeKind(experimentId,
			...
			experimentValue = createExperimentValueByExperimentIdAndStateTypeKindAndValueTypeKind(experimentId,
	...
	private ExperimentValue createExperimentValueByExperimentIdAndStateTypeKindAndValueTypeKind(
		...
		ExperimentState experimentState = ExperimentState
				.findExperimentStatesByExptIDAndStateTypeKind(experimentId, stateType, stateKind).getSingleResult();
		experimentValue.setLsState(experimentState);
		...
		if (valueType.equals("fileValue"))
			experimentValue.setFileValue(value);
		if (valueType.equals("clobValue"))
			experimentValue.setClobValue(value);
		if (valueType.equals("blobValue"))
			experimentValue.setBlobValue(value.getBytes(Charset.forName("UTF-8")));
		if (valueType.equals("numericValue"))
			experimentValue.setNumericValue(new BigDecimal(value));
		if (valueType.equals("dateValue"))
			experimentValue.setDateValue(new Date(Long.parseLong(value)));
		if (valueType.equals("codeValue"))
			experimentValue.setCodeValue(value);
		if (valueType.equals("urlValue"))
			experimentValue.setUrlValue(value);
	...
	public ExperimentValue getExperimentValue(String idOrCodeName,
		...
		try {
			Collection<ExperimentValue> values = getExperimentValues(idOrCodeName, stateType, stateKind, valueType,
					valueKind);
	...
	public Collection<ExperimentValuePathDTO> getExperimentValues(
		...
		for (GenericValuePathRequest request : genericRequests) {
			...
			result.setValues(getExperimentValues(request.getIdOrCodeName(), request.getStateType(),
	...
	private Collection<ExperimentValue> getExperimentValues(String idOrCodeName, String stateType, String stateKind,
			String valueType, String valueKind) {
		if (SimpleUtil.isNumeric(idOrCodeName)) {
			Long id = Long.valueOf(idOrCodeName);
			return ExperimentValue.findExperimentValuesByExptIDAndStateTypeKindAndValueTypeKind(id, stateType,
					stateKind, valueType, valueKind).getResultList();
		} else {
			return ExperimentValue.findExperimentValuesByExperimentCodeNameAndStateTypeKindAndValueTypeKind(
