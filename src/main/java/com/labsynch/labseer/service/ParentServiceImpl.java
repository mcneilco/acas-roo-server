...
public class ParentServiceImpl implements ParentService {
	...
	public ParentValidationDTO validateUniqueParent(Parent queryParent) throws CmpdRegMolFormatException {
		...
		if (queryParent.getCorpName() == null)
			validationDTO.getErrors()
					.add(new ErrorMessage("error", "Must provide corpName for parent to be validated"));
		if (queryParent.getStereoCategory() == null)
			validationDTO.getErrors()
					.add(new ErrorMessage("error", "Must provide stereo category for parent to be validated"));
		if (queryParent.getStereoCategory().getCode().equalsIgnoreCase("see_comments")
				&& (queryParent.getStereoComment() == null || queryParent.getStereoComment().length() == 0)) {
			validationDTO.getErrors()
					.add(new ErrorMessage("error", "Stereo category is See Comments, but no stereo comment provided"));
		}
		if (chemStructureService.checkForSalt(queryParent.getMolStructure())) {
			if (queryParent.getIsMixture() != null) {
				if (!queryParent.getIsMixture()) {
					validationDTO.getErrors().add(new ErrorMessage("error",
							"Multiple fragments found. Please register the neutral base parent or mark as a Mixture"));
				}
			} else {
				validationDTO.getErrors().add(new ErrorMessage("error",
						"Multiple fragments found. Please register the neutral base parent or mark as a Mixture"));
		...
		if (propertiesUtilService.getRegisterNoStructureCompoundsAsUniqueParents()
				&& chemStructureService.isEmpty(queryParent.getMolStructure())) {
			logger.warn(
			...
			searchResultLoop: for (int foundParentCdId : dupeParentList) {
				...
				for (Parent foundParent : foundParents) {
					...
					if (queryParent.getCorpName().equals(foundParent.getCorpName())) {
						...
						if (queryParent.getStereoCategory().getCode()
							...
							} else if (queryParent.getStereoComment() == null
									|| foundParent.getStereoComment() == null) {
								// one null, the other is not => not a dupe
								continue;
							} else if (queryParent.getStereoComment()
									.equalsIgnoreCase(foundParent.getStereoComment())) {
	...
	public Parent updateParentMeta(ParentEditDTO parentDTO) {
		...
		if (parentDTO.getComment() != null)
			parent.setComment(parentDTO.getComment());
		if (parentDTO.getStereoCategoryCode() != null && parentDTO.getStereoCategoryCode().length() > 0) {
			parent.setStereoCategory(StereoCategory.findStereoCategorysByCodeEquals(parentDTO.getStereoCategoryCode())
					.getSingleResult());
		}
		if (parentDTO.getStereoComment() != null)
			parent.setStereoComment(parentDTO.getStereoComment());
		...
		if (parentDTO.getCommonName() != null && parentDTO.getCommonName().length() > 0)
			parent.setCommonName(parentDTO.getCommonName());
		if (parentDTO.getIgnore() != null)
			parent.setIgnore(parentDTO.getIgnore());
		if (parentDTO.getIsMixture() != null)
			parent.setIsMixture(parentDTO.getIsMixture());
		if (parentDTO.getCompoundTypeCode() != null && parentDTO.getCompoundTypeCode().length() > 0) {
			parent.setCompoundType(
					CompoundType.findCompoundTypesByCodeEquals(parentDTO.getCompoundTypeCode()).getSingleResult());
		}
		if (parentDTO.getParentAnnotationCode() != null && parentDTO.getParentAnnotationCode().length() > 0) {
			parent.setParentAnnotation(ParentAnnotation
					.findParentAnnotationsByCodeEquals(parentDTO.getParentAnnotationCode()).getSingleResult());
		}
		...
		if (parentValidationDTO.isParentUnique()) {
			...
			if (parentDTO.getModifiedBy() != null && parentDTO.getModifiedBy().length() > 0)
