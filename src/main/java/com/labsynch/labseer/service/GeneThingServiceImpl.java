...
public class GeneThingServiceImpl implements GeneThingService {
	...
	public void RegisterGeneThingsFromCSV(String inputFile) throws IOException {
		...
		try {
			...
			while ((geneDTO = beanReader.read(EntrezDbGeneDTO.class, header, processors)) != null) {
				...
				Long geneIdCount = LsThingLabel.countOfLsThingByName(geneTypeString, geneKindString, "name",
						"Entrez Gene ID", geneDTO.getGeneId());
				...
				if (geneIdCount == 0L) {
					...
					saveGeneDescriptor(lsTransaction, geneState, "stringValue", "chromosome", geneDTO.getChromosome(),
							entrezSeparator);
					saveGeneDescriptor(lsTransaction, geneState, "stringValue", "map location",
							geneDTO.getMapLocation(), entrezSeparator);
	...
	public LsThingValue saveGeneDescriptor(LsTransaction lsTransaction, LsThingState geneState, String lsType,
			String lsKind, Date dateValue) {
	...
	public List<LsThingValue> saveGeneDescriptor(LsTransaction lsTransaction, LsThingState geneState, String lsType,
			String lsKind, String dataValues, String entrezSeparator) {
	...
	public List<LsThingLabel> saveGeneLabel(LsTransaction lsTransaction, LsThing geneThing, String labelType,
		...
		if (labels != null) {
			...
			for (String label : labelList) {
				if (label.equalsIgnoreCase("-")) {
					...
					// if (LsThingLabel.countOfLsThingLabelsByLabel(geneThing, labelType, labelKind,
					// label) == 0) {
	...
	public LsThing registerNewGene(EntrezDbGeneDTO geneDTO, LsTransaction lsTransaction) {
		...
		Long geneIdCount = LsThingLabel.countOfLsThingByName(geneTypeString, geneKindString, "name", "Entrez Gene ID",
				geneDTO.getGeneId());
		LsThing geneThing = null;

		if (geneIdCount == 0L) {
			geneThing = new LsThing();
			geneThing.setCodeName(
					autoLabelService.getAutoLabels(thingTypeAndKind, labelTypeAndKind, 1L).get(0).getAutoLabel());
			geneThing.setLsType(geneTypeString);
			geneThing.setLsKind(geneKindString);
			geneThing.setLsTransaction(lsTransaction.getId());
			geneThing.setRecordedBy("acas admin");
			...
			saveGeneLabel(lsTransaction, geneThing, "name", "authoritative gene symbol", false,
					geneDTO.getSymbolFromAuthority(), entrezSeparator);
			saveGeneLabel(lsTransaction, geneThing, "name", "other designations", false, geneDTO.getOtherDesignations(),
					entrezSeparator);
			...
			// saveGeneLabel(lsTransaction, geneThing, "name", "Entrez Gene ID", true,
			// geneDTO.getGeneId());
	...
	@Override
	@Transactional
	public void updateEntrezGenes(String entrezGenesFile, String geneHistoryFile, String taxonomyId)
			throws IOException {
	...
	private void updateGeneHistoryEntities(String geneHistoryFile, String taxonomyId, LsTransaction lsTransaction)
			throws IOException {
		...
		try {
			...
			while ((geneDTO = beanReader.read(EntrezDiscontinuedGeneDTO.class, header, processors)) != null) {
				logger.debug(String.format("lineNo=%s, rowNo=%s, bulkData=%s", beanReader.getLineNumber(),
						beanReader.getRowNumber(), geneDTO));
	...
	private LsThing updateMergedGene(EntrezDiscontinuedGeneDTO geneDTO, LsTransaction lsTransaction) {
		...
		List<LsThing> oldGenes = LsThing.findAllLsThingByLabelTypeAndKindAndLabelText("gene", "entrez gene", "name",
				"Entrez Gene ID", geneDTO.getDiscontinuedGeneId()).getResultList();
		List<LsThing> activeGenes = LsThing.findAllLsThingByPreferredLabelText("gene", "entrez gene", "name",
				"Entrez Gene ID", geneDTO.getGeneId()).getResultList();
		// List<LsThing> activeGenes =
		// LsThing.findAllLsThingByPreferredLabelText("gene", "entrez gene", "name",
		// "Entrez Gene ID", geneDTO.getGeneId()).getResultList();
		...
		if (oldGenes.size() == 0 && activeGenes.size() == 0) {
			// check by any label
			List<LsThing> allGenes = LsThing.findAllLsThingByLabelTypeAndKindAndLabelText("gene", "entrez gene", "name",
					"Entrez Gene ID", geneDTO.getGeneId()).getResultList();
			if (allGenes.size() > 0) {
				logger.info(
						"Found a gene entity by non-preferred id  -- could be a previously merged gene.  Number of genes found: "
								+ allGenes.size());
				...
				if (logger.isDebugEnabled())
					logger.debug("registering new gene: " + geneDTO.getGeneId());
				...
				activeGenes = LsThing.findAllLsThingByPreferredLabelText("gene", "entrez gene", "name",
						"Entrez Gene ID", geneDTO.getGeneId()).getResultList();
				oldGenes = LsThing.findAllLsThingByLabelTypeAndKindAndLabelText("gene", "entrez gene", "name",
						"Entrez Gene ID", geneDTO.getDiscontinuedGeneId()).getResultList();
			}
		...
		if (oldGenes.size() == 0 && activeGenes.size() == 1) {
			// add discontinued Gene ID to the active Gene -- unless it already has the gene
			// ID
			geneThing = activeGenes.get(0);

			// check if old gene ID exists, if not create it
			Long countOfGeneID = LsThingLabel.countOfLsThingLabelsByLabel(geneThing, "name", "Entrez Gene ID",
					geneDTO.getDiscontinuedGeneId());
			if (countOfGeneID == 0) {
				geneThing.getLsLabels().add(saveGeneLabel(lsTransaction, geneThing, "name", "Entrez Gene ID", false,
						geneDTO.getDiscontinuedGeneId()));
			}

			Long countOfGeneSymbol = LsThingLabel.countOfLsThingLabelsByLabel(geneThing, "name", "gene symbol",
					geneDTO.getDiscontinuedSymbol());
			if (countOfGeneSymbol == 0) {
				geneThing.getLsLabels().add(saveGeneLabel(lsTransaction, geneThing, "name", "gene symbol", false,
						geneDTO.getDiscontinuedSymbol()));
			}
			...
			if (sameGene) {
				...
				List<LsThingLabel> oldGeneIdLabels = LsThingLabel
						.findLsThingLabelsByLsThingAndLsTypeAndLsKindAndLabelText(oldGene, "name", "Entrez Gene ID",
								geneDTO.getGeneId())
						.getResultList();
				if (oldGeneIdLabels.size() == 0) {
					oldGene.getLsLabels().add(saveGeneLabel(lsTransaction, oldGene, "name", "Entrez Gene ID", false,
							geneDTO.getGeneId()));
				}

				List<LsThingLabel> newGeneIdLabels = LsThingLabel
						.findLsThingLabelsByLsThingAndLsTypeAndLsKindAndLabelText(geneThing, "name", "Entrez Gene ID",
								geneDTO.getDiscontinuedGeneId())
						.getResultList();
			...
			List<LsThing> geneThingsAll = LsThing.findAllLsThingByLabelTypeAndKindAndLabelText("gene", "entrez gene",
					"name", "Entrez Gene ID", geneDTO.getGeneId()).getResultList();
	...
	private LsThing registerDiscontinuedGeneNew(EntrezDiscontinuedGeneDTO geneDTO, LsTransaction lsTransaction) {
		...
		List<LsThing> genes = LsThing.findAllLsThingByLabelTypeAndKindAndLabelText("gene", "entrez gene", "name",
				"Entrez Gene ID", geneDTO.getDiscontinuedGeneId()).getResultList();
		...
		if (genes.size() == 0) {
			...
			String entrezSeparator = "\\|";
			geneThing.getLsLabels().addAll(saveGeneLabel(lsTransaction, geneThing, "name", "Entrez Gene ID", true,
					geneDTO.getDiscontinuedGeneId(), entrezSeparator));
			geneThing.getLsLabels().addAll(saveGeneLabel(lsTransaction, geneThing, "name", "gene symbol", false,
					geneDTO.getDiscontinuedSymbol(), entrezSeparator));
			...
			geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue", "tax id",
					geneDTO.getTaxId(), entrezSeparator));
			geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue",
					"discontinued gene", "true", entrezSeparator));
			geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue",
					"discontinued gene id", geneDTO.getDiscontinuedGeneId(), entrezSeparator));
			geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue",
					"discontinued gene symbol", geneDTO.getDiscontinuedSymbol(), entrezSeparator));
			geneState.getLsValues().add(saveGeneDescriptor(lsTransaction, geneState, "dateValue", "discontinued date",
					geneDTO.getDiscontinuedDate()));
			...
			List<LsThingLabel> geneLabels = LsThingLabel
					.findLsThingPreferredName(geneThing.getId(), "name", "Entrez Gene ID").getResultList();
	...
	public void updateEntrezGeneEntities(String entrezGenesFile, String taxonomyId, LsTransaction lsTransaction)
			throws IOException {
		...
		try {
			...
			while ((geneDTO = beanReader.read(EntrezDbGeneDTO.class, header, processors)) != null) {
				...
				// check if gene currently exists
				Long geneIdCount = LsThingLabel.countOfAllLsThingByPreferredName(geneTypeString, geneKindString, "name",
						"Entrez Gene ID", geneDTO.getGeneId());
				logger.debug("Process Entrez Genes ------------------- geneId count " + geneIdCount);

				// may need to see if the gene ID exists on another gene -- not as a preferred
				...
				if (geneIdCount == 0L) {
					...
					if (logger.isDebugEnabled())
						logger.debug("gene already registered: " + geneDTO.getGeneId());
	...
	private LsThing updateExistingGene(EntrezDbGeneDTO geneDTO, LsTransaction lsTransaction) {
		...
		List<LsThing> genes = LsThing.findAllLsThingByPreferredLabelText(geneTypeString, geneKindString, "name",
				"Entrez Gene ID", geneDTO.getGeneId()).getResultList();
		...
		if (genes.size() == 0) {
			...
			List<LsThingValue> dateValues = LsThingValue.findLsThingValuesByLsThingIDAndStateTypeKindAndValueTypeKind(
					geneThing.getId(), "metadata", "gene metadata", "dateValue", "modification date").getResultList();
			logger.debug("number of mod dates found: " + dateValues.size());
			if (dateValues.size() > 1)
				logger.error("Found more than 1 mod date: " + dateValues.size());
	...
	// public LsThing updateGene(EntrezDbGeneDTO geneDTO, EntrezDiscontinuedGeneDTO
	// entrezDiscontinuedGeneDTO, LsTransaction lsTransaction) {
	// String geneTypeString = "gene";
	// String geneKindString = "entrez gene";
	// List<LsThing> genes = LsThing.findLsThingByAllLabelText(geneTypeString,
	// geneKindString, "name", "Entrez Gene ID",
	// entrezDiscontinuedGeneDTO.getDiscontinuedGeneId()).getResultList();
	// List<LsThing> activeGenes =
	// LsThing.findLsThingByPreferredLabelText(geneTypeString, geneKindString,
	// "name", "Entrez Gene ID", geneDTO.getGeneId()).getResultList();
	//
	// LsThing geneThing = null ;
	//
	// logger.debug("Searching for discontinued gene: " +
	// entrezDiscontinuedGeneDTO.getDiscontinuedGeneId());
	// logger.debug("updateGene: number of discontinued genes found: " +
	// genes.size());
	// logger.debug("Searching for active gene: " + geneDTO.getGeneId());
	// logger.debug("updateGene: number of active genes found: " +
	// activeGenes.size());
	//
	//
	// if (genes.size() == 0){
	...
	// } else {
	// geneThing = genes.get(0);
	// geneThing = updateGeneID(geneThing, geneDTO, entrezDiscontinuedGeneDTO,
	// lsTransaction);
	// geneThing = updateGeneMetaData(geneThing, geneDTO, lsTransaction);
	// }
	//
	...
	public LsThing updateGeneMetaData(LsThing geneThing, EntrezDbGeneDTO geneDTO, LsTransaction lsTransaction) {
		...
		String entrezSeparator = "\\|";
		existingLabels = saveGeneLabel(existingLabels, lsTransaction, geneThing, "name", "gene symbol", false,
				geneDTO.getSymbol(), entrezSeparator);
		existingLabels = saveGeneLabel(existingLabels, lsTransaction, geneThing, "name", "gene synonym", false,
				geneDTO.getSynonyms(), entrezSeparator);
		existingLabels = saveGeneLabel(existingLabels, lsTransaction, geneThing, "name", "authoritative gene name",
				false, geneDTO.getFullNameFromAuthority(), entrezSeparator);
		existingLabels = saveGeneLabel(existingLabels, lsTransaction, geneThing, "name", "authoritative gene symbol",
				false, geneDTO.getSymbolFromAuthority(), entrezSeparator);
		existingLabels = saveGeneLabel(existingLabels, lsTransaction, geneThing, "name", "other designations", false,
				geneDTO.getOtherDesignations(), entrezSeparator);
		...
		// harder to parse changing labels between updates, i.e. authorized symbols and
		// etc
		...
		// } else {
		//
		// logger.debug("@@@@@@@@@@@@@@ IGNORING GENE LABELS @@@@@@@@@@@@@ GeneThing: "
		// + geneThing.getCodeName() + " " + geneLabel.getLabelText() );
		...
		// }
		//
		// // geneThing.getLsLabels().addAll(saveGeneLabel(lsTransaction, geneThing,
		// "name", "Entrez Gene ID", true, geneDTO.getGeneId(), entrezSeparator));
		// geneThing.getLsLabels().addAll(saveGeneLabel(lsTransaction, geneThing,
		// "name", "gene symbol", false, geneDTO.getSymbol(), entrezSeparator));
		// geneThing.getLsLabels().addAll(saveGeneLabel(lsTransaction, geneThing,
		// "name", "gene synonym", false, geneDTO.getSynonyms(), entrezSeparator));
		// geneThing.getLsLabels().addAll(saveGeneLabel(lsTransaction, geneThing,
		// "name", "authoritative gene name", false, geneDTO.getFullNameFromAuthority(),
		// entrezSeparator));
		// geneThing.getLsLabels().addAll(saveGeneLabel(lsTransaction, geneThing,
		// "name", "authoritative gene symbol", false, geneDTO.getSymbolFromAuthority(),
		// entrezSeparator));
		// geneThing.getLsLabels().addAll(saveGeneLabel(lsTransaction, geneThing,
		// "name", "other designations", false, geneDTO.getOtherDesignations(),
		// entrezSeparator));
		...
		geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue", "tax id",
				geneDTO.getTaxId(), entrezSeparator));
		geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue", "locus tag",
				geneDTO.getLocusTag(), entrezSeparator));
		geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue", "dbXrefs",
				geneDTO.getDbXrefs(), entrezSeparator));
		geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue", "chromosome",
				geneDTO.getChromosome(), entrezSeparator));
		geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue", "map location",
				geneDTO.getMapLocation(), entrezSeparator));
		geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue", "description",
				geneDTO.getDescription(), entrezSeparator));
		geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue", "type of gene",
				geneDTO.getTypeOfGene(), entrezSeparator));
		geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue",
				"nomenclature status", geneDTO.getNomenclatureStatus(), entrezSeparator));
		geneState.getLsValues().add(saveGeneDescriptor(lsTransaction, geneState, "dateValue", "modification date",
				geneDTO.getModificationDate()));
	...
	public LsThing discontinueGene(LsThing inactiveGene, EntrezDiscontinuedGeneDTO discontinuedGeneDTO,
	...
	public LsThing addDiscontinueMetadata(LsThing geneThing, EntrezDiscontinuedGeneDTO entrezDiscontinuedGeneDTO,
			LsTransaction lsTransaction) {

		List<LsThingValue> thingValues = LsThingValue
				.findLsThingValuesByLsThingIDAndStateTypeKindAndValueTypeKind(geneThing.getId(), "metadata",
						"gene discontinued metadata", "stringValue", "discontinued gene")
				.getResultList();
		...
		if (!discontinued) {
			...
			geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue",
					"discontinued gene", "true", entrezSeparator));
			geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue",
					"discontinued gene id", entrezDiscontinuedGeneDTO.getDiscontinuedGeneId(), entrezSeparator));
			geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState, "stringValue",
					"discontinued gene symbol", entrezDiscontinuedGeneDTO.getDiscontinuedSymbol(), entrezSeparator));
			geneState.getLsValues().add(saveGeneDescriptor(lsTransaction, geneState, "dateValue", "discontinued date",
					entrezDiscontinuedGeneDTO.getDiscontinuedDate()));
			geneThing.getLsStates().add(geneState);
		}
	...
	public GeneHistoryDTO processGeneHistory(String geneHistoryFileName, String taxonomyId) throws IOException {
		...
		// HashMap<String, EntrezDiscontinuedGeneDTO> updatedGenes = new HashMap<String,
		// EntrezDiscontinuedGeneDTO>();
		// HashMap<String, EntrezDiscontinuedGeneDTO> discontinuedGenes = new
		// HashMap<String, EntrezDiscontinuedGeneDTO>();
		...
		try {
			...
			// List<EntrezDiscontinuedGeneDTO> updatedGeneList =
			// (List<EntrezDiscontinuedGeneDTO>) updatedGenes.get(queryGeneId);
	...
	// for (EntrezDiscontinuedGeneDTO singleGene : (List<EntrezDiscontinuedGeneDTO>)
	// updatedGenes.get(geneIdKey)){
	...
	// }
	//
	// private LsThing updateGeneLabels(EntrezDiscontinuedGeneDTO singleGene,
	// LsTransaction lsTransaction) {
	//
	// List<LsThing> discontinuedGenes = LsThing.findLsThingByLabelText("gene",
	// "entrez gene", "name", "Entrez Gene ID",
	// singleGene.getDiscontinuedGeneId()).getResultList();
	// List<LsThing> genes = LsThing.findLsThingByLabelText("gene", "entrez gene",
	// "name", "Entrez Gene ID", singleGene.getGeneId()).getResultList();
	//
	// int discontinuedGenesCount = discontinuedGenes.size();
	// int geneCount = genes.size();
	//
	// logger.debug("------------ the gene ID count is: " + discontinuedGenes.size()
	// + " for discontinued gene " + singleGene.getDiscontinuedGeneId());
	// logger.debug("------------ the gene ID count is: " + genes.size() + " for new
	// gene " + singleGene.getGeneId());
	//
	// LsThing geneThing = null ;
	...
	//
	// //check if old gene ID exists, if not create it
	// Long countOfGeneID = LsThingLabel.countOfLsThingLabelsByLabel(geneThing,
	// "name", "Entrez Gene ID", singleGene.getDiscontinuedGeneId());
	// Long countOfGeneSymbol = LsThingLabel.countOfLsThingLabelsByLabel(geneThing,
	// "name", "gene symbol", singleGene.getDiscontinuedSymbol());
	// if (countOfGeneID == 0){
	// geneThing.getLsLabels().add(saveGeneLabel(lsTransaction, geneThing, "name",
	// "Entrez Gene ID", false, singleGene.getDiscontinuedGeneId()));
	// }
	// if (countOfGeneSymbol == 0){
	// geneThing.getLsLabels().add(saveGeneLabel(lsTransaction, geneThing, "name",
	// "gene symbol", false, singleGene.getDiscontinuedSymbol()));
	// }
	//
	...
	// @SuppressWarnings("unchecked")
	// public void processEntrezGenes(String entrezGenes, GeneHistoryDTO
	// historyResults,
	...
	// geneBeanMap.put("type_of_gene", "typeOfGene");
	// geneBeanMap.put("Symbol_from_nomenclature_authority", "symbolFromAuthority");
	// geneBeanMap.put("Full_name_from_nomenclature_authority",
	// "fullNameFromAuthority");
	// geneBeanMap.put("Nomenclature_status", "nomenclatureStatus");
	// geneBeanMap.put("Other_designations", "otherDesignations");
	// geneBeanMap.put("Modification_date", "modificationDate");
	//
	...
	// // Long numberOfLabels = new Long(numberOfLines-1);
	// //
	// // List<AutoLabelDTO> thingCodes =
	// autoLabelService.getAutoLabels(thingTypeAndKind, labelTypeAndKind,
	// numberOfLabels);
	//
	// int i = 0;
	// int batchSize = 25;
	// LsThing geneThing = null;
	// while( (geneDTO = beanReader.read(EntrezDbGeneDTO.class, header, processors))
	// != null ) {
	// System.out.println(String.format("lineNo=%s, rowNo=%s, bulkData=%s",
	// beanReader.getLineNumber(), beanReader.getRowNumber(), geneDTO));
	...
	// // check if gene currently exists
	// Long geneIdCount =
	// LsThingLabel.countOfAllLsThingByPreferredName(geneTypeString, geneKindString,
	// "name", "Entrez Gene ID", geneDTO.getGeneId());
	// // Long oldGeneIdCount = LsThingLabel.countOfLsThingByName(geneTypeString,
	// geneKindString, "name", "Entrez Gene ID",
	// historyResults.getUpdatedGenes().get(geneDTO.getGeneId()).getDiscontinuedGeneId());
	//
	// logger.debug("Process Entrez Genes ------------------- geneId count " +
	// geneIdCount);
	//
	//
	...
	// // 1. gene ID not registered and in list of discontinued genes --> register
	// discontinued gene
	// // 2. gene ID not registered and in list of updated genes --> update gene
	// entry;
	// // set old gene ID to not preferred,
	// // add new gene ID as the preferred;
	// // ignore old gene metadata,
	// // add new gene metadata
	// // 3. index through discontinued genes -- register all genes not already
	// registered
	//
	// // for (){
	// //
	...
	// for (EntrezDiscontinuedGeneDTO singleGene : (List<EntrezDiscontinuedGeneDTO>)
	// historyResults.getDiscontinuedGenes().get(geneDTO.getGeneId())){
	// geneThing = registerDiscontinuedGene(singleGene, lsTransaction);
	// }
	//
	// } else if (updatedGene ){
	// // updating existing gene info with new gene label and meta (merging genes if
	// new gene does not exist)
	// // else keep 2 gene entities
	//
	// for (EntrezDiscontinuedGeneDTO singleGene : (List<EntrezDiscontinuedGeneDTO>)
	// historyResults.getUpdatedGenes().get(geneDTO.getGeneId())){
	// geneThing = registerUpdatedGene(geneDTO, singleGene,
	// (List<EntrezDiscontinuedGeneDTO>)
	// historyResults.getUpdatedGenes().get(geneDTO.getGeneId()), lsTransaction);
	// if (logger.isDebugEnabled()) logger.debug("updated the gene entry: " +
	// LsThing.findLsThing(geneThing.getId()).toPrettyJson());
	// }
	//
	// } else if (geneIdCount == 0L ) {
	// // brand new gene to register
	// if (logger.isDebugEnabled()) logger.debug("registering new gene: " +
	// geneDTO.getGeneId());
	// geneThing = registerNewGene(geneDTO, lsTransaction);
	//
	// } else {
	// if (logger.isDebugEnabled()) logger.debug("gene already registered: " +
	// geneDTO.getGeneId());
	// }
	//
	// if ( i % batchSize == 0 ) { // same as the JDBC batch size
	...
	// EntrezDiscontinuedGeneDTO singleDiscontinuedGene,
	// List<EntrezDiscontinuedGeneDTO> discontinuedGeneList, LsTransaction
	// lsTransaction) {
	...
	//
	// // use case -- user wants to see all the data for gene id 11797; 11797 maps
	// back to discontinued gene 77616 and 102757
	// // gene A has a gene ID label for 11797 and 10257
	// // gene B has a gene ID label for 77616
	// // AG data for both geneA and geneB.
	// // option 1: rename the batch code for geneB to geneA. (actual change of
	// data)
	// // option 2: leave the AG data alone. If user queries for data for 11797 --
	// we must also return data for 77616
	// // so gene A should have a label for the discontinued gene id:
	// name_discontinued Entrez Gene ID? or just another Entrez Gene ID not
	// preferred
	// // in some cases -- there may or may not be another gene thing with a
	// matching preferred Entrez Gene ID
	// // let's work with option 2 for now. So genes will also have a label of
	// discontinued gene id -- mark more info in comments?
	//
	// String geneTypeString = "gene";
	// String geneKindString = "entrez gene";
	// // Long geneIdCount = LsThingLabel.countOfLsThingByName(geneTypeString,
	// geneKindString, "name", "Entrez Gene ID",
	// entrezDiscontinuedGeneDTO.getDiscontinuedGeneId());
	// // Long newGeneIdCount = LsThingLabel.countOfLsThingByName(geneTypeString,
	// geneKindString, "name", "Entrez Gene ID", geneDTO.getGeneId());
	//
	// // things = LsThing.findLsThingByLabelKindAndLabelText(geneTypeString,
	// geneKindString, labelKind, labelText);
	//
	// logger.debug("searching for new genes by: " + geneDTO.getGeneId());
	// List<LsThing> newGenes =
	// LsThing.findAllLsThingByPreferredLabelText(geneTypeString, geneKindString,
	// "name", "Entrez Gene ID", geneDTO.getGeneId()).getResultList();
	//
	// logger.debug("searching for discontinued genes by: " +
	// singleDiscontinuedGene.getDiscontinuedGeneId());
	// List<LsThing> discontinuedGenes = LsThing.findLsThingByLabelText("gene",
	// "entrez gene", "name", "Entrez Gene ID",
	// singleDiscontinuedGene.getDiscontinuedGeneId()).getResultList();
	//
	//
	...
	//
	// logger.debug("------------ the gene ID count is: " + discontinuedGenes.size()
	// + " for discontinued gene " +
	// singleDiscontinuedGene.getDiscontinuedGeneId());
	// logger.debug("------------ the gene ID count is: " + newGenes.size() + " for
	// new gene " + geneDTO.getGeneId());
	//
	// //are the genes the same?
	// boolean sameGene = false;
	// if (discontinuedGenes.size() == 1 && newGenes.size() == 1){
	// if (discontinuedGenes.get(0).getId().longValue() ==
	// newGenes.get(0).getId().longValue()){
	// logger.debug("@@@@@@ Mapping back to the same gene @@@@@@ " +
	// discontinuedGenes.get(0).getCodeName());
	// sameGene = true;
	// }
	// }
	//
	...
	// logger.debug("gene is not registered yet");
	// // geneThing = registerNewGene(geneDTO, singleDiscontinuedGene,
	// lsTransaction);
	// geneThing = registerNewGene(geneDTO, lsTransaction);
	//
	// } else if (geneIdCount == 1 && newGeneIdCount == 1 && sameGene){
	// logger.debug("------ both gene IDs are currently registered with the same
	// geneThing");
	// // add discontinued meta to the old gene
	//
	//
	...
	// //if (logger.isDebugEnabled()) logger.debug("added discontinued gene meta to
	// the old gene: " + oldGene.toPrettyJson());
	//
	// // update the gene with the new info
	// List<LsThing> genes = LsThing.findLsThingByPreferredLabelText(geneTypeString,
	// geneKindString, "name", "Entrez Gene ID",
	// geneDTO.getGeneId()).getResultList();
	// if (genes.size() == 0){
	// logger.error("Gene not found");
	// } else if (genes.size() > 1){
	// logger.error("multiple genes found with the same gene ID");
	...
	// //check the modification date
	// boolean isNewDate = false;
	// List<LsThingValue> dateValues =
	// LsThingValue.findLsThingValuesByLsThingIDAndStateTypeKindAndValueTypeKind(geneThing.getId(),
	// "metadata", "gene metadata", "dateValue", "modification
	// date").getResultList();
	// logger.debug("number of mod dates found: " + dateValues.size());
	// if (dateValues.size() > 1) logger.error("Found more than 1 mod date: " +
	// dateValues.size());
	// for (LsThingValue dateValue : dateValues){
	// if (geneDTO.getModificationDate().after(dateValue.getDateValue())){
	// isNewDate = true;
	...
	// logger.debug("adding old gene ID " + discontinuedGene.getDiscontinuedGeneId()
	// + " to the gene: " + geneThing.getCodeName());
	// geneThing = addOldGeneID(geneThing, discontinuedGene, lsTransaction);
	//
	// //add reference gene to the old Gene as well
	// logger.debug("adding reference gene ID " + geneDTO.getGeneId() + " to the
	// following old gene: " + discontinuedGene.getDiscontinuedGeneId());
	// addReferenceGeneIdToDiscontinuedGene(geneDTO, discontinuedGene,
	// lsTransaction);
	// }
	//
	//
	...
	// List<LsThing> genes = LsThing.findLsThingByLabelText(geneTypeString,
	// geneKindString, "name", "Entrez Gene ID",
	// geneDTO.getGeneId()).getResultList();
	// if (genes.size() == 0){
	// logger.error("Gene not found");
	...
	// } else {
	// geneThing = genes.get(0);
	// geneThing = addOldGeneID(geneThing, geneDTO, singleDiscontinuedGene,
	// lsTransaction);
	// }
	// } else {
	// logger.debug("-------- gene is registered via old id geneIdCount: " +
	// geneIdCount + " newGeneIdCount: " + newGeneIdCount);
	//
	// // if (logger.isDebugEnabled()) logger.debug("old gene: " +
	// LsThing.findLsThingByLabelText(geneTypeString, geneKindString, "name",
	// "Entrez Gene ID",
	// entrezDiscontinuedGeneDTO.getDiscontinuedGeneId()).getResultList().get(0).toPrettyJson());
	//
	//
	// List<LsThing> genes = LsThing.findLsThingByPreferredLabelText(geneTypeString,
	// geneKindString, "name", "Entrez Gene ID",
	...
	// } else {
	// geneThing = genes.get(0);
	// logger.debug("working with gene thing: " + geneThing.getCodeName() + " linked
	// to Gene ID: " + geneDTO.getGeneId());
	// }
	//
	//
	// //check the modification date
	...
	// // saveGeneLabel(lsTransaction, geneThing, "name", "Entrez Gene ID", true,
	// geneDTO.getGeneId(), entrezSeparator)
	// }
	//
	// //for loop
	// for (EntrezDiscontinuedGeneDTO discontinuedGene : discontinuedGeneList){
	...
	// private void addReferenceGeneIdToDiscontinuedGene(EntrezDbGeneDTO geneDTO,
	// EntrezDiscontinuedGeneDTO discontinuedGene, LsTransaction lsTransaction) {
	// List<LsThing> inactiveGenes =
	// LsThing.findAllLsThingByPreferredLabelText("gene", "entrez gene", "name",
	// "Entrez Gene ID", discontinuedGene.getDiscontinuedGeneId()).getResultList();
	// logger.debug("@@@@@@@@@ number of inactive genes found: " +
	// inactiveGenes.size() + " for gene " +
	// discontinuedGene.getDiscontinuedGeneId());
	// for (LsThing inactiveGene : inactiveGenes){
	//
	// boolean activeGene = false;
	// List<LsThingLabel> lsThingLabels =
	// LsThingLabel.findLsThingPreferredName(inactiveGene.getId(), "name", "Entrez
	// Gene ID").getResultList();
	...
	// }
	// List<LsThingLabel> geneIdLabels =
	// LsThingLabel.findLsThingLabelsByLsThingAndLsTypeAndLsKindAndLabelText(inactiveGene,
	// "name", "Entrez Gene ID", geneDTO.getGeneId()).getResultList();
	// if (geneIdLabels.size() == 0){
	// saveGeneLabel(lsTransaction, inactiveGene, "name", "Entrez Gene ID", false,
	// geneDTO.getGeneId());
	// // inactiveGene.getLsLabels().add(saveGeneLabel(lsTransaction, inactiveGene,
	// "name", "Entrez Gene ID", false, geneDTO.getGeneId()));
	// }
	// }
	// }
	// }

	// private LsThing addOldGeneID(LsThing geneThing, EntrezDiscontinuedGeneDTO
	// discontinuedGene, LsTransaction lsTransaction) {
	// List<LsThingLabel> geneIdLabels =
	// LsThingLabel.findLsThingLabelsByLsThingAndLsTypeAndLsKindAndLabelText(geneThing,
	// "name", "Entrez Gene ID",
	// discontinuedGene.getDiscontinuedGeneId()).getResultList();
	// if (geneIdLabels.size() == 0){
	// geneThing.getLsLabels().add(saveGeneLabel(lsTransaction, geneThing, "name",
	// "Entrez Gene ID", false, discontinuedGene.getDiscontinuedGeneId()));
	// }
	//
	...
	// public LsThing addOldGeneID(LsThing geneThing, EntrezDbGeneDTO geneDTO,
	// EntrezDiscontinuedGeneDTO entrezDiscontinuedGeneDTO, LsTransaction
	// lsTransaction) {
	// if (entrezDiscontinuedGeneDTO.getDiscontinuedGeneId() != null)
	// geneThing.getLsLabels().add(saveGeneLabel(lsTransaction, geneThing, "name",
	// "Entrez Gene ID", false, entrezDiscontinuedGeneDTO.getDiscontinuedGeneId()));
	// if (entrezDiscontinuedGeneDTO.getDiscontinuedSymbol() != null)
	// geneThing.getLsLabels().add(saveGeneLabel(lsTransaction, geneThing, "name",
	// "gene symbol", false, entrezDiscontinuedGeneDTO.getDiscontinuedSymbol()));
	// return geneThing;
	// }
	//
	...
	//
	//
	// public LsThing ignoreNonGeneIDLabels(LsTransaction lsTransaction, LsThing
	// geneThing) {
	...
	// }
	//
	// public LsThing updateGeneID(LsThing geneThing, EntrezDbGeneDTO geneDTO,
	// EntrezDiscontinuedGeneDTO entrezDiscontinuedGeneDTO, LsTransaction
	// lsTransaction) {
	//
	...
	// geneLabel.getLsKind().equalsIgnoreCase("Entrez Gene ID")
	// &&
	// geneLabel.getLabelText().equalsIgnoreCase(entrezDiscontinuedGeneDTO.getDiscontinuedGeneId())
	// && geneLabel.isPreferred()){
	// geneLabel.setPreferred(false);
	// }
	// }
	//
	// LsThingLabel newGeneLabel = saveGeneLabel(lsTransaction, geneThing, "name",
	// "Entrez Gene ID", true, entrezDiscontinuedGeneDTO.getGeneId());
	...
	//
	// String entrezSeparator = "\\|";
	// geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState,
	// "stringValue", "discontinued gene id",
	// entrezDiscontinuedGeneDTO.getDiscontinuedGeneId(), entrezSeparator));
	// geneState.getLsValues().addAll(saveGeneDescriptor(lsTransaction, geneState,
	// "stringValue", "discontinued gene symbol",
	// entrezDiscontinuedGeneDTO.getDiscontinuedSymbol(), entrezSeparator));
	// geneState.getLsValues().add(saveGeneDescriptor(lsTransaction, geneState,
	// "dateValue", "discontinued date",
	// entrezDiscontinuedGeneDTO.getDiscontinuedDate()));
	// geneThing.getLsStates().add(geneState);
	//
	// return geneThing;
	...
	// }
	//
	// public LsThing registerDiscontinuedGene(EntrezDiscontinuedGeneDTO geneDTO,
	// LsTransaction lsTransaction) {
	// logger.debug("create discontinued gene: " + geneDTO.getDiscontinuedGeneId());
	// String geneTypeString = "gene";
	// String geneKindString = "entrez gene";
	...
	// saveGeneLabel(lsTransaction, geneThing, "name", "Entrez Gene ID", true,
	// geneDTO.getDiscontinuedGeneId(), entrezSeparator);
	// saveGeneLabel(lsTransaction, geneThing, "name", "gene symbol", false,
	// geneDTO.getDiscontinuedSymbol(), entrezSeparator);
	//
	// LsThingState geneState = new LsThingState();
	// geneState.setLsThing(geneThing);
	...
	// geneState.persist();
	//
	// saveGeneDescriptor(lsTransaction, geneState, "stringValue", "tax id",
	// geneDTO.getTaxId(), entrezSeparator);
	// saveGeneDescriptor(lsTransaction, geneState, "stringValue", "discontinued
	// gene", "true", entrezSeparator);
	// saveGeneDescriptor(lsTransaction, geneState, "stringValue", "discontinued
	// gene id", geneDTO.getDiscontinuedGeneId(), entrezSeparator);
	// saveGeneDescriptor(lsTransaction, geneState, "stringValue", "discontinued
	// gene symbol", geneDTO.getDiscontinuedSymbol(), entrezSeparator);
	// saveGeneDescriptor(lsTransaction, geneState, "dateValue", "discontinued
	// date", geneDTO.getDiscontinuedDate());
	//
	// return geneThing;
	// }
	//
	...
	// List<EntrezDiscontinuedGeneDTO> discontinuedGeneSet =
	// (List<EntrezDiscontinuedGeneDTO>) discontinuedGenes.get(key);
	...
	// logger.info("-----------------------------------------------current
	// discontinued gene update count: " + geneCount);
	// if (geneThing != null){
	// geneThing.flush();
	// geneThing.clear();
	// }
	...
	// }
	//
	// public LsThing createDiscontinuedGene(EntrezDiscontinuedGeneDTO geneDTO,
	// LsTransaction lsTransaction) {
	// logger.debug("create discontinued gene: " + geneDTO.getDiscontinuedGeneId());
	// String geneTypeString = "gene";
	// String geneKindString = "entrez gene";
	...
	// @SuppressWarnings("unchecked")
	// @Override
	// public void fixDiscontinuedEntrezGeneIDs(String geneHistoryFile, String
	// taxonomyId) throws IOException {
	//
	// //get historyResults
	...
	// // mark the other labels as ignored (we will only refer to this gene entity
	// by the old Gene ID
	// // keep the discontinued gene id that was assigned to the new gene. (allows
	// us to tie the genes together -- need to make changes in the preferred ID
	// service)
	//
	...
	// lsTransaction.persist();
	//
	// GeneHistoryDTO historyResults = processGeneHistory(geneHistoryFile,
	// taxonomyId);
	...
	// List<EntrezDiscontinuedGeneDTO> updatedGeneList =
	// (List<EntrezDiscontinuedGeneDTO>)
	// historyResults.getUpdatedGenes().get(geneId);
	...
	//
	// List<LsThing> genes = LsThing.findLsThingByAllLabelText("gene", "entrez
	// gene", "name", "Entrez Gene ID", geneId).getResultList();
	...
	// }
	//
	// if (genes.size() > 2) throw new RuntimeException("found multiple gene things
	// >2: " + genes.size());
	//
	// if (genes.size() > 1){
	// for (LsThing gene : genes){
	...
	// }
	// // List<LsThingLabel> preferredGeneNames =
	// LsThingLabel.findLsThingAllPreferredName(gene.getId(), "name", "Entrez Gene
	// ID").getResultList();
	...
	// // logger.error("found many preferredGeneNames: " +
	// preferredGeneNames.size());
	// // } else {
	...
	// }
	//
	// curateOldGene(geneId, targetLabel, lowestGeneThingId, highestGeneThingId,
	// activeGeneIds, lsTransaction);
	//
	// if (discontinuedGeneIds.contains(geneId)){
	// logger.info("---------- removing discontinued gene labels for gene id: " +
	// geneId);
	...
	// if (logger.isDebugEnabled()){
	// logger.debug("----------- displaying lowest gene thing id: " +
	// lowestGeneThingId + " -----------------------");
	// displayGeneLabels(lowestGeneThingId);
	// logger.debug("----------- displaying highest gene thing id: " +
	// highestGeneThingId + " -----------------------");
	...
	// for (LsThing gene : genes){
	// List<LsThingLabel>preferredGeneNames =
	// LsThingLabel.findLsThingPreferredName(gene.getId(), "name", "Entrez Gene
	// ID").getResultList();
	// if (preferredGeneNames.size() == 1){
	...
	// if (gene.isIgnored()){
	// logger.info(" @@@@@@@@@@@@@@@@@@@@@ ----- UN-ignoring the gene: " +
	// gene.getId());
	...
	// @SuppressWarnings("unchecked")
	// private void curateOldGene(String geneId, LsThingLabel targetLabel, long
	// lowestGeneThingId, long highestGeneThingId, Collection<String> activeGeneIds,
	// LsTransaction lsTransaction) {
	//
	// LsThing newGene = LsThing.findLsThing(lowestGeneThingId);
	// String newPreferredId = targetLabel.getLabelText();
	// List<LsThingLabel> newGeneLabels =
	// LsThingLabel.findLsThingLabelsByLsThingAndLsTypeAndLsKind(newGene, "name",
	// "Entrez Gene ID").getResultList();
	...
	//
	// List<LsThingLabel> oldGeneLabels =
	// LsThingLabel.findLsThingLabelsByLsThingAndLsTypeAndLsKind(oldGene, "name",
	// "Entrez Gene ID").getResultList();
	...
	// logger.info("@@@@@@@@@@@@ IGNORED GENE THING @@@@@@@@@@ " +
	// highestGeneThingId);
	...
	// logger.debug("----------- Adding gene label to new Gene: " +
	// oldGeneLabel.getLabelText());
	...
	// private void setPreferredName(long lowestGeneThingId, String geneId,
	// LsTransaction lsTransaction) {
	// logger.debug("Setting the preferred entrez gene ID for gene thing id: " +
	// lowestGeneThingId);
	// @SuppressWarnings("unchecked")
	// List<LsThingLabel> thingLabels =
	// LsThingLabel.findLsThingLabelsByLsThingAndLsTypeAndLsKindAndLabelText(LsThing.findLsThing(lowestGeneThingId),
	// "name", "Entrez Gene ID", geneId).getResultList();
	...
	// }
	// } else {
	// logger.debug("number of entrez gene ids to sort through: " +
	// thingLabels.size());
	// List<LsThingLabel> otherThingLabels =
	// LsThingLabel.findLsThingLabelsByLsThingAndLsTypeAndLsKind(LsThing.findLsThing(lowestGeneThingId),
	// "name", "Entrez Gene ID").getResultList();
	//
	// boolean firstPass = true;
	...
	// logger.debug("@@@@@@@ REMOVING Preferred GENE ID @@@@@@@ " +
	// thingLabel.toJson() );
	// //thingLabel.remove();
	...
	// Collection<LsThingLabel> labels =
	// LsThingLabel.findLsThingLabelsByLsThing(LsThing.findLsThing(geneThingId)).getResultList();
	// Map<String, LsThingLabel> labelMap = new HashMap<String, LsThingLabel>();
	// String labelKey;
	// for (LsThingLabel label : labels){
	// labelKey = new
	// StringBuilder().append(label.getLsType()).append("_").append(label.getLsKind()).append("_").append(label.getLabelText()).toString();
	...
	// // Collection<LsThingLabel> oldGeneLabels =
	// LsThingLabel.findLsThingLabelsByLsThing(LsThing.findLsThing(highestGeneThingId)).getResultList();
	...
	// // }
	//
	// Collection<LsThingLabel> newGeneLabels =
	// LsThingLabel.findLsThingLabelsByLsThing(LsThing.findLsThing(geneThingId)).getResultList();
	...
	// Collection<LsThingLabel> labels =
	// LsThingLabel.findLsThingLabelsByLsThing(gene).getResultList();
	// for (LsThingLabel label : labels){
	// logger.debug("current label ID: " + label.getId() + " label: " +
	// label.getLabelText());
	...
	private void displayGeneLabels(long geneThingId) {
		Collection<LsThingLabel> labels = LsThingLabel.findLsThingLabelsByLsThing(LsThing.findLsThing(geneThingId))
				.getResultList();
	...
	@Override
	@Transactional
	public LsThing saveOrthologEntity(String versionName, String testFileName, String orthologType, Long curationLevel,
		...
		if (recordedBy.isEmpty() || recordedBy.equalsIgnoreCase(""))
			recordedBy = "acas-admin";
	...
	@Override
	public void registerGeneOrthologsFromCSV(String inputFile, String ortCodeName, String recordedBy,
			Long lsTransactionId) throws IOException {
		...
		while ((geneOrthologDTO = beanReader.read(GeneOrthologDTO.class, headerArray, processors)) != null) {
			// System.out.println(String.format("lineNo=%s, rowNo=%sd() + " mapped Gene " +
			// geneOrthologDTO.getMappedGeneId, bulkData=%s", beanReader.getLineNumber(),
			// beanReader.getRowNumber(), geneOrthologDTO));
			logger.info("current gene: " + geneOrthologDTO.getGeneId());

			LsThing targetGene = null;
			try {
				targetGene = LsThing.findLsThingByLabelText("gene", "entrez gene", "name", "Entrez Gene ID",
						geneOrthologDTO.getGeneId()).getSingleResult();
			...
			try {
				orthologGene = LsThing.findLsThingByLabelText("gene", "entrez gene", "name", "Entrez Gene ID",
						geneOrthologDTO.getMappedGeneId()).getSingleResult();
			...
			// itxOrtholog.setCodeName(autoLabelService.getLsThingCodeName("ortholog
			// of_entrez gene_entrez gene"));
			...
			savedValue = saveItxStateCodeValue(lsTransaction, recordedBy, itxOrthologState, ortCodeName, "ortholog",
					"entrez gene", "ACAS LsThing", "codeValue", "ortholog");

			savedValue = saveItxStateValue(lsTransaction, recordedBy, itxOrthologState, "numericValue", "MappedScore",
					geneOrthologDTO.getMappedScore());
			savedValue = saveItxStateValue(lsTransaction, recordedBy, itxOrthologState, "numericValue", "MappedPerc",
					geneOrthologDTO.getMappedPerc());
			savedValue = saveItxStateValue(lsTransaction, recordedBy, itxOrthologState, "numericValue", "MappedHitLen",
					geneOrthologDTO.getMappedHitLen());
			savedValue = saveItxStateValue(lsTransaction, recordedBy, itxOrthologState, "numericValue",
					"MappedPercRatio", geneOrthologDTO.getMappedPercRatio());

			// savedValue = saveItxStateValue(lsTransaction, recordedBy, itxOrthologState,
			// "stringValue", "InputGeneId", geneOrthologDTO.getGeneId());
			// savedValue = saveItxStateValue(lsTransaction, recordedBy, itxOrthologState,
			// "stringValue", "InputSymbol", geneOrthologDTO.getSymbol());
			// savedValue = saveItxStateValue(lsTransaction, recordedBy, itxOrthologState,
			// "stringValue", "InputTaxId", geneOrthologDTO.getTaxId());
			// savedValue = saveItxStateValue(lsTransaction, recordedBy, itxOrthologState,
			// "stringValue", "MappedGeneId", geneOrthologDTO.getMappedGeneId());
			// savedValue = saveItxStateValue(lsTransaction, recordedBy, itxOrthologState,
			// "stringValue", "MappedSymbol", geneOrthologDTO.getMappedSymbol());
			// savedValue = saveItxStateValue(lsTransaction, recordedBy, itxOrthologState,
			// "stringValue", "MappedTaxId", geneOrthologDTO.getMappedTaxId());
	...
	private ItxLsThingLsThingValue saveItxStateCodeValue(LsTransaction lsTransaction, String recordedBy,
			ItxLsThingLsThingState itxOrthologState, String codeValue, String codeType, String codeKind,
			String codeOrigin,
	...
	public ItxLsThingLsThingValue saveItxStateValue(LsTransaction lsTransaction, String recordedBy,
			ItxLsThingLsThingState itxOrthologState, String stateType, String stateKind, Integer numericValue) {
		ItxLsThingLsThingValue savedValue = new ItxLsThingLsThingValue();
	...
	public ItxLsThingLsThingValue saveItxStateValue(LsTransaction lsTransaction, String recordedBy,
			ItxLsThingLsThingState itxOrthologState, String stateType, String stateKind, Double numericValue) {
		ItxLsThingLsThingValue savedValue = new ItxLsThingLsThingValue();
		savedValue.setLsType(stateType);
		savedValue.setLsKind(stateKind);
		savedValue.setLsTransaction(lsTransaction.getId());
	...
	public ItxLsThingLsThingValue saveItxStateValue(LsTransaction lsTransaction, String recordedBy,
			ItxLsThingLsThingState itxOrthologState, String stateType, String stateKind, String stringValue) {
		ItxLsThingLsThingValue savedValue = new ItxLsThingLsThingValue();
		savedValue.setLsType(stateType);
		savedValue.setLsKind(stateKind);
		savedValue.setLsTransaction(lsTransaction.getId());
	...
	public void setGeneOrthologDefaults() {
		...
		List<LabelSequence> ortxLabelSeqs = LabelSequence
				.findLabelSequencesByThingTypeAndKindEqualsAndLabelTypeAndKindEquals(thingTypeAndKind, labelTypeAndKind)
				.getResultList();
	...
	public List<GeneOrthologDTO> getOrthologsFromGeneID(String queryGeneID) throws IOException {
		...
		List<LsThing> queryGenes = LsThing
				.findLsThingByLabelText("gene", "entrez gene", "name", "Entrez Gene ID", queryGeneID).getResultList();
		...
		List<ItxLsThingLsThing> itxOrthologs = ItxLsThingLsThing
				.findItxLsThingLsThingsByLsTypeEqualsAndLsKindEqualsAndFirstLsThingEquals(lsType, lsKind, queryGene)
				.getResultList();
	...
	private String findItxStateValueString(ItxLsThingLsThing itxOrtholog, String valueKind) {
		...
		List<ItxLsThingLsThingValue> itxStateValues = ItxLsThingLsThingValue
				.findItxLsValueByItxThingAndStateTypeStateKindAndValueTypeKind(itxOrtholog.getId(), stateType,
	...
	private GeneOrthologDTO setOrthologInfo(GeneOrthologDTO geneOrtholog) {
		...
		List<LsThingLabel> orthologNames = LsThingLabel
				.findLsThingLabelsByLsThingAndLsTypeAndLsKindNotIgnored(orthologThing, "name", "ortholog name", true)
				.getResultList();
		...
		List<LsThingValue> curationLevels = LsThingValue.findLsThingValuesByLsThingIDAndStateTypeKindAndValueTypeKind(
				orthologThing.getId(), "metadata", "ortholog", "numericValue", "curation level").getResultList();
		...
		List<LsThingValue> curators = LsThingValue.findLsThingValuesByLsThingIDAndStateTypeKindAndValueTypeKind(
				orthologThing.getId(), "metadata", "ortholog", "stringValue", "curator").getResultList();
		if (curators.size() == 1) {
	...
	private boolean checkValidOrtholog(ItxLsThingLsThing itxOrtholog) {
		...
		List<ItxLsThingLsThingValue> orthCodes = ItxLsThingLsThingValue
				.findItxLsValueByItxThingAndStateTypeStateKindAndValueTypeKind(itxOrtholog.getId(), stateType,
						stateKind, valueType, valueKind)
				.getResultList();
	...
	private String findVersionName(GeneOrthologDTO geneOrtholog) {
		...
		if (geneOrtholog.getOrthCode() != null) {
			...
			List<LsThingLabel> orthologNames = LsThingLabel
					.findLsThingLabelsByLsThingAndLsTypeAndLsKindNotIgnored(orthThing, "name", "ortholog name", true)
					.getResultList();
			if (orthologNames.size() == 1) {
	...
	private Integer findCurationLevel(ItxLsThingLsThing itxOrtholog) {
		...
		List<ItxLsThingLsThingValue> curationLevels = ItxLsThingLsThingValue
				.findItxLsValueByItxThingAndStateTypeStateKindAndValueTypeKind(itxOrtholog.getId(), stateType,
						stateKind, valueType, valueKind)
				.getResultList();
	...
	private String findOrthologCurator(ItxLsThingLsThing itxOrtholog) {
		...
		List<ItxLsThingLsThingValue> curators = ItxLsThingLsThingValue
				.findItxLsValueByItxThingAndStateTypeStateKindAndValueTypeKind(itxOrtholog.getId(), stateType,
						stateKind, valueType, valueKind)
				.getResultList();
	...
	private String findGeneID(LsThing mappedOrthologGene) {
		String geneID = null;
		List<LsThingLabel> geneIDs = LsThingLabel
				.findLsThingPreferredName(mappedOrthologGene.getId(), "name", "Entrez Gene ID", true, true)
				.getResultList();
		if (geneIDs.size() == 1) {
	...
	private String findTaxonomyID(LsThing queryGene) {
		...
		List<LsThingValue> taxIdValues = LsThingValue.findLsThingValuesByLsThingIDAndStateTypeKindAndValueTypeKind(
				queryGene.getId(), "metadata", "gene metadata", "stringValue", "tax id").getResultList();
	...
	private String findGeneSymbol(LsThing queryGene) {
		...
		List<LsThingLabel> geneSymbols = LsThingLabel.findLsThingLabelsByLsThingAndLsTypeAndLsKindNotIgnored(queryGene,
				"name", "authoritative gene symbol", true).getResultList();
	...
	public GeneOrthologDTO saveOrthologInteraction(GeneOrthologDTO geneOrtholog) {
		...
		if (geneOrtholog.getId() == null) {
			// this is a new entry
			List<LsThing> queryGenes = LsThing
					.findLsThingByLabelText("gene", "entrez gene", "name", "Entrez Gene ID", geneOrtholog.getGeneId())
					.getResultList();
			...
			List<LsThing> orthologGenes = LsThing.findLsThingByLabelText("gene", "entrez gene", "name",
					"Entrez Gene ID", geneOrtholog.getMappedGeneId()).getResultList();
			LsThing orthologGene = null;
			...
			ItxLsThingLsThingValue savedValue;
			savedValue = saveItxStateValue(lsTransaction, geneOrtholog.getRecordedBy(), itxOrthologState,
					"numericValue", "curation level", geneOrtholog.getCurationLevel());
			savedValue = saveItxStateValue(lsTransaction, geneOrtholog.getRecordedBy(), itxOrthologState, "stringValue",
					"curator", geneOrtholog.getCurator());
			if (geneOrtholog.getVersionName() != null) {
				savedValue = saveItxStateValue(lsTransaction, geneOrtholog.getRecordedBy(), itxOrthologState,
						"stringValue", "version name", geneOrtholog.getVersionName());
			}
