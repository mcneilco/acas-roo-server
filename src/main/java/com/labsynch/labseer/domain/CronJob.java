...
public class CronJob {
    ...
    public static final List<String> fieldNames4OrderClauseFilter = java.util.Arrays.asList("logger", "schedule",
            "scriptType", "scriptFile", "functionName", "scriptJSONData", "active", "ignored", "runUser", "codeName",
            "lastStartTime", "lastDuration", "lastResultJSON", "numberOfExecutions", "id", "version");
    ...
    public static List<CronJob> findCronJobEntries(int firstResult, int maxResults) {
        return entityManager().createQuery("SELECT o FROM CronJob o", CronJob.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }

    public static List<CronJob> findCronJobEntries(int firstResult, int maxResults, String sortFieldName,
            String sortOrder) {
        String jpaQuery = "SELECT o FROM CronJob o";
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            jpaQuery = jpaQuery + " ORDER BY " + sortFieldName;
        ...
        return entityManager().createQuery(jpaQuery, CronJob.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }
    ...
    public static TypedQuery<CronJob> findCronJobsByCodeNameEquals(String codeName) {
        if (codeName == null || codeName.length() == 0)
            throw new IllegalArgumentException("The codeName argument is required");
        EntityManager em = CronJob.entityManager();
        TypedQuery<CronJob> q = em.createQuery("SELECT o FROM CronJob AS o WHERE o.codeName = :codeName",
                CronJob.class);
        q.setParameter("codeName", codeName);
        return q;
    }

    public static TypedQuery<CronJob> findCronJobsByCodeNameEquals(String codeName, String sortFieldName,
            String sortOrder) {
        if (codeName == null || codeName.length() == 0)
            throw new IllegalArgumentException("The codeName argument is required");
        EntityManager em = CronJob.entityManager();
