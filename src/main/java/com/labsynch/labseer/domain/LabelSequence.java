...
public class LabelSequence {
    ...
    public LabelSequence save() {
        ...
        if (this.getDbSequence() == null) {
            // set to temp sequence first so we can persist, get and id and then use the id
            // to create a unique sequence name
            this.setDbSequence("tempseq");
            this.persist();
            String dbSequence = "labelseq_" + this.getId() + "_" + this.getLabelPrefix() + "_"
                    + this.getLabelTypeAndKind() + "_" + this.getThingTypeAndKind();
            dbSequence = dbSequence.replaceAll("[^a-zA-Z0-9_]+", "_");
            // Limit to 30 characters to be compatible with Oracle Version <= 12.1 if
            // applicable
        ...
        if (this.getStartingNumber() < 1L)
            this.setStartingNumber(1L);
        Query q = em
                .createNativeQuery("CREATE SEQUENCE " + this.dbSequence + " START WITH " + this.getStartingNumber());
        q.executeUpdate();
        this.merge();
        return this;
    }
    ...
    public AutoLabelDTO generateNextLabel() {
        ...
        String label = this.getLabelPrefix().concat(this.getLabelSeparator())
                .concat(String.format(formatLabelNumber, labelNumber));
            ...
            public Long execute(Connection connection) throws SQLException {
                ...
                try {
                    ...
                    if (dbType == DbType.POSTGRES) {
                        ...
                        currentValueString = "SELECT last_number FROM all_sequences WHERE sequence_name = '"
                                + sequenceName + "'";
                    }
                    try (
    ...
    public String toJson() {
        return new JSONSerializer().exclude("*.class", "labelSequenceRoles.labelSequenceEntry")
                .include("labelSequenceRoles.roleEntry").transform(new ExcludeNulls(), void.class).serialize(this);
    }

    public static String toJsonArray(Collection<LabelSequence> collection) {
        return new JSONSerializer().exclude("*.class", "labelSequenceRoles.labelSequenceEntry")
                .include("labelSequenceRoles.roleEntry").transform(new ExcludeNulls(), void.class)
                .serialize(collection);
    }

    public static Long countFindLabelSequencesByLabelTypeAndKindEquals(String labelTypeAndKind) {
        if (labelTypeAndKind == null || labelTypeAndKind.length() == 0)
            throw new IllegalArgumentException("The labelTypeAndKind argument is required");
        EntityManager em = LabelSequence.entityManager();
        TypedQuery q = em.createQuery(
                "SELECT COUNT(o) FROM LabelSequence AS o WHERE o.labelTypeAndKind = :labelTypeAndKind", Long.class);
    ...
    public static Long countFindLabelSequencesByThingTypeAndKindEquals(String thingTypeAndKind) {
        if (thingTypeAndKind == null || thingTypeAndKind.length() == 0)
            throw new IllegalArgumentException("The thingTypeAndKind argument is required");
        EntityManager em = LabelSequence.entityManager();
        TypedQuery q = em.createQuery(
                "SELECT COUNT(o) FROM LabelSequence AS o WHERE o.thingTypeAndKind = :thingTypeAndKind", Long.class);
        q.setParameter("thingTypeAndKind", thingTypeAndKind);
        return ((Long) q.getSingleResult());
    }

    public static Long countFindLabelSequencesByThingTypeAndKindEqualsAndLabelTypeAndKindEquals(String thingTypeAndKind,
        ...
        EntityManager em = LabelSequence.entityManager();
        TypedQuery q = em.createQuery(
                "SELECT COUNT(o) FROM LabelSequence AS o WHERE o.thingTypeAndKind = :thingTypeAndKind  AND o.labelTypeAndKind = :labelTypeAndKind",
                Long.class);
        q.setParameter("thingTypeAndKind", thingTypeAndKind);
        q.setParameter("labelTypeAndKind", labelTypeAndKind);
        return ((Long) q.getSingleResult());
    ...
    public static Long countFindLabelSequencesByThingTypeAndKindEqualsAndLabelTypeAndKindEqualsAndLabelPrefixEquals(
            String thingTypeAndKind, String labelTypeAndKind, String labelPrefix) {
        if (thingTypeAndKind == null || thingTypeAndKind.length() == 0)
            throw new IllegalArgumentException("The thingTypeAndKind argument is required");
        if (labelTypeAndKind == null || labelTypeAndKind.length() == 0)
            throw new IllegalArgumentException("The labelTypeAndKind argument is required");
        if (labelPrefix == null || labelPrefix.length() == 0)
            throw new IllegalArgumentException("The labelPrefix argument is required");
        EntityManager em = LabelSequence.entityManager();
        TypedQuery q = em.createQuery(
                "SELECT COUNT(o) FROM LabelSequence AS o WHERE o.thingTypeAndKind = :thingTypeAndKind  AND o.labelTypeAndKind = :labelTypeAndKind  AND o.labelPrefix = :labelPrefix",
                Long.class);
        q.setParameter("thingTypeAndKind", thingTypeAndKind);
        q.setParameter("labelTypeAndKind", labelTypeAndKind);
    ...
    public static TypedQuery<LabelSequence> findLabelSequencesByLabelTypeAndKindEquals(String labelTypeAndKind) {
        if (labelTypeAndKind == null || labelTypeAndKind.length() == 0)
            throw new IllegalArgumentException("The labelTypeAndKind argument is required");
        EntityManager em = LabelSequence.entityManager();
        TypedQuery<LabelSequence> q = em.createQuery(
                "SELECT o FROM LabelSequence AS o WHERE o.labelTypeAndKind = :labelTypeAndKind", LabelSequence.class);
        q.setParameter("labelTypeAndKind", labelTypeAndKind);
        return q;
    }
    ...
    public static TypedQuery<LabelSequence> findLabelSequencesByLabelTypeAndKindEquals(String labelTypeAndKind,
        ...
        StringBuilder queryBuilder = new StringBuilder(
                "SELECT o FROM LabelSequence AS o WHERE o.labelTypeAndKind = :labelTypeAndKind");
    ...
    public static TypedQuery<LabelSequence> findLabelSequencesByThingTypeAndKindEquals(String thingTypeAndKind) {
        ...
        EntityManager em = LabelSequence.entityManager();
        TypedQuery<LabelSequence> q = em.createQuery(
                "SELECT o FROM LabelSequence AS o WHERE o.thingTypeAndKind = :thingTypeAndKind", LabelSequence.class);
        q.setParameter("thingTypeAndKind", thingTypeAndKind);
        return q;
    }
    ...
    public static TypedQuery<LabelSequence> findLabelSequencesByThingTypeAndKindEquals(String thingTypeAndKind,
        ...
        EntityManager em = LabelSequence.entityManager();
        StringBuilder queryBuilder = new StringBuilder(
                "SELECT o FROM LabelSequence AS o WHERE o.thingTypeAndKind = :thingTypeAndKind");
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            queryBuilder.append(" ORDER BY ").append(sortFieldName);
            if ("ASC".equalsIgnoreCase(sortOrder) || "DESC".equalsIgnoreCase(sortOrder)) {
                queryBuilder.append(" ").append(sortOrder);
    ...
    public static TypedQuery<LabelSequence> findLabelSequencesByThingTypeAndKindEqualsAndLabelTypeAndKindEquals(
            String thingTypeAndKind, String labelTypeAndKind) {
        if (thingTypeAndKind == null || thingTypeAndKind.length() == 0)
            throw new IllegalArgumentException("The thingTypeAndKind argument is required");
        if (labelTypeAndKind == null || labelTypeAndKind.length() == 0)
            throw new IllegalArgumentException("The labelTypeAndKind argument is required");
        EntityManager em = LabelSequence.entityManager();
        TypedQuery<LabelSequence> q = em.createQuery(
                "SELECT o FROM LabelSequence AS o WHERE o.thingTypeAndKind = :thingTypeAndKind  AND o.labelTypeAndKind = :labelTypeAndKind",
                LabelSequence.class);
        q.setParameter("thingTypeAndKind", thingTypeAndKind);
        q.setParameter("labelTypeAndKind", labelTypeAndKind);
        return q;
    ...
    public static TypedQuery<LabelSequence> findLabelSequencesByThingTypeAndKindEqualsAndLabelTypeAndKindEquals(
            String thingTypeAndKind, String labelTypeAndKind, String sortFieldName, String sortOrder) {
        if (thingTypeAndKind == null || thingTypeAndKind.length() == 0)
            throw new IllegalArgumentException("The thingTypeAndKind argument is required");
        if (labelTypeAndKind == null || labelTypeAndKind.length() == 0)
            throw new IllegalArgumentException("The labelTypeAndKind argument is required");
    ...
    public static TypedQuery<LabelSequence> findLabelSequencesByThingTypeAndKindEqualsAndLabelTypeAndKindEqualsAndLabelPrefixEquals(
            String thingTypeAndKind, String labelTypeAndKind, String labelPrefix) {
        if (thingTypeAndKind == null || thingTypeAndKind.length() == 0)
            throw new IllegalArgumentException("The thingTypeAndKind argument is required");
        if (labelTypeAndKind == null || labelTypeAndKind.length() == 0)
        ...
        EntityManager em = LabelSequence.entityManager();
        TypedQuery<LabelSequence> q = em.createQuery(
                "SELECT o FROM LabelSequence AS o WHERE o.thingTypeAndKind = :thingTypeAndKind  AND o.labelTypeAndKind = :labelTypeAndKind  AND o.labelPrefix = :labelPrefix",
                LabelSequence.class);
        q.setParameter("thingTypeAndKind", thingTypeAndKind);
        q.setParameter("labelTypeAndKind", labelTypeAndKind);
        q.setParameter("labelPrefix", labelPrefix);
    ...
    public static TypedQuery<LabelSequence> findLabelSequencesByThingTypeAndKindEqualsAndLabelTypeAndKindEqualsAndLabelPrefixEquals(
            String thingTypeAndKind, String labelTypeAndKind, String labelPrefix, String sortFieldName,
            String sortOrder) {
        if (thingTypeAndKind == null || thingTypeAndKind.length() == 0)
            throw new IllegalArgumentException("The thingTypeAndKind argument is required");
        if (labelTypeAndKind == null || labelTypeAndKind.length() == 0)
    ...
    public static final List<String> fieldNames4OrderClauseFilter = java.util.Arrays.asList("thingTypeAndKind",
            "labelTypeAndKind", "labelPrefix", "labelSeparator", "groupDigits", "digits", "startingNumber", "ignored",
            "modifiedDate", "dbSequence", "labelSequenceRoles");
    ...
    public static List<LabelSequence> findLabelSequenceEntries(int firstResult, int maxResults) {
        return entityManager().createQuery("SELECT o FROM LabelSequence o", LabelSequence.class)
                .setFirstResult(firstResult).setMaxResults(maxResults).getResultList();
    }

    public static List<LabelSequence> findLabelSequenceEntries(int firstResult, int maxResults, String sortFieldName,
            String sortOrder) {
        String jpaQuery = "SELECT o FROM LabelSequence o";
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            jpaQuery = jpaQuery + " ORDER BY " + sortFieldName;
        ...
        return entityManager().createQuery(jpaQuery, LabelSequence.class).setFirstResult(firstResult)
