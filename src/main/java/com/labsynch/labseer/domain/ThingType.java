...
public class ThingType {
    ...
    public static List<ThingType> findThingTypeEntries(int firstResult, int maxResults) {
        return entityManager().createQuery("SELECT o FROM ThingType o", ThingType.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }

    public static List<ThingType> findThingTypeEntries(int firstResult, int maxResults, String sortFieldName,
            String sortOrder) {
        String jpaQuery = "SELECT o FROM ThingType o";
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            jpaQuery = jpaQuery + " ORDER BY " + sortFieldName;
        ...
        return entityManager().createQuery(jpaQuery, ThingType.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }
    ...
    public static TypedQuery<ThingType> findThingTypesByTypeNameEquals(String typeName) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = ThingType.entityManager();
        TypedQuery<ThingType> q = em.createQuery("SELECT o FROM ThingType AS o WHERE o.typeName = :typeName",
                ThingType.class);
        q.setParameter("typeName", typeName);
        return q;
    }

    public static TypedQuery<ThingType> findThingTypesByTypeNameEquals(String typeName, String sortFieldName,
            String sortOrder) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = ThingType.entityManager();
