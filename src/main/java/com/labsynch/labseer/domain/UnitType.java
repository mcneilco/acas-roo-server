...
public class UnitType {
    ...
    public static List<UnitType> findUnitTypeEntries(int firstResult, int maxResults) {
        return entityManager().createQuery("SELECT o FROM UnitType o", UnitType.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }

    public static List<UnitType> findUnitTypeEntries(int firstResult, int maxResults, String sortFieldName,
            String sortOrder) {
        String jpaQuery = "SELECT o FROM UnitType o";
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            jpaQuery = jpaQuery + " ORDER BY " + sortFieldName;
        ...
        return entityManager().createQuery(jpaQuery, UnitType.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }
    ...
    public static TypedQuery<UnitType> findUnitTypesByTypeNameEquals(String typeName) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = UnitType.entityManager();
        TypedQuery<UnitType> q = em.createQuery("SELECT o FROM UnitType AS o WHERE o.typeName = :typeName",
                UnitType.class);
        q.setParameter("typeName", typeName);
        return q;
    }

    public static TypedQuery<UnitType> findUnitTypesByTypeNameEquals(String typeName, String sortFieldName,
            String sortOrder) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = UnitType.entityManager();
