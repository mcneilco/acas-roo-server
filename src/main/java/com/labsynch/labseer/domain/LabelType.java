...
public class LabelType {
    ...
    public static List<LabelType> findLabelTypeEntries(int firstResult, int maxResults) {
        return entityManager().createQuery("SELECT o FROM LabelType o", LabelType.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }

    public static List<LabelType> findLabelTypeEntries(int firstResult, int maxResults, String sortFieldName,
            String sortOrder) {
        String jpaQuery = "SELECT o FROM LabelType o";
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            jpaQuery = jpaQuery + " ORDER BY " + sortFieldName;
        ...
        return entityManager().createQuery(jpaQuery, LabelType.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }
    ...
    public static TypedQuery<LabelType> findLabelTypesByTypeNameEquals(String typeName) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = LabelType.entityManager();
        TypedQuery<LabelType> q = em.createQuery("SELECT o FROM LabelType AS o WHERE o.typeName = :typeName",
                LabelType.class);
        q.setParameter("typeName", typeName);
        return q;
    }

    public static TypedQuery<LabelType> findLabelTypesByTypeNameEquals(String typeName, String sortFieldName,
            String sortOrder) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = LabelType.entityManager();
