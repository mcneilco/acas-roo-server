...
public class Subject extends AbstractThing {
    ...
    @JoinTable(name = "TREATMENTGROUP_SUBJECT", joinColumns = {
            @JoinColumn(name = "subject_id") }, inverseJoinColumns = { @JoinColumn(name = "treatment_group_id") })
    ...
    @OneToMany(cascade = { CascadeType.PERSIST,
            CascadeType.MERGE }, mappedBy = "subject", fetch = FetchType.LAZY, orphanRemoval = true)
    ...
    // return new JSONSerializer()
    // .include("lsLabels", "lsStates.lsValues")
    // .exclude("*.class", "lsStates.subject", "lsStates.lsValues.lsState",
    // "treatmentGroup.analysisGroup")
    // .serialize(this);
    // }

    // .include("subjectLabels", "subjectStates.subjectValues", "containers")
    // .include("subjectLabels", "subjectStates.subjectValues")
    // .exclude("*.class", "subjectLabels.subject", "subjectStates.subject",
    // "subjectStates.subjectValues.subjectState", "treatmentGroup.analysisGroup")

    public static Subject fromJsonToSample(String json) {
        return new JSONDeserializer<Subject>().use(null, Subject.class).
    ...
    public static String toJsonArray(Collection<Subject> collection) {
        return new JSONSerializer()
                .include("lsLabels", "lsStates.lsValues")
                .exclude("*.class", "lsLabels.subject", "lsStates.subject", "lsStates.lsValues.lsState",
                        "treatmentGroup.analysisGroup")
    ...
    public static Collection<Subject> fromJsonArrayToSubjects(String json) {
        return new JSONDeserializer<List<Subject>>().use(null, ArrayList.class).use("values", Subject.class)
                .deserialize(json);
    }
    ...
    public static Subject findSubjectByCodeNameEquals(String codeName) {
        ...
        EntityManager em = Subject.entityManager();
        TypedQuery<Subject> q = em.createQuery("SELECT o FROM Subject AS o WHERE o.codeName = :codeName",
                Subject.class);
        q.setParameter("codeName", codeName);
        return q.getSingleResult();
    }

    public static final List<String> fieldNames4OrderClauseFilter = java.util.Arrays.asList("logger", "treatmentGroups",
            "lsLabels", "lsStates", "containers");
    ...
    public static List<Subject> findSubjectEntries(int firstResult, int maxResults) {
        return entityManager().createQuery("SELECT o FROM Subject o", Subject.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }

    public static List<Subject> findSubjectEntries(int firstResult, int maxResults, String sortFieldName,
            String sortOrder) {
        String jpaQuery = "SELECT o FROM Subject o";
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            jpaQuery = jpaQuery + " ORDER BY " + sortFieldName;
        ...
        return entityManager().createQuery(jpaQuery, Subject.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }
    ...
    public static Long countFindSubjectsByLsTransactionEquals(Long lsTransaction) {
        ...
        EntityManager em = Subject.entityManager();
        TypedQuery q = em.createQuery("SELECT COUNT(o) FROM Subject AS o WHERE o.lsTransaction = :lsTransaction",
                Long.class);
    ...
    public static Long countFindSubjectsByTreatmentGroups(Set<TreatmentGroup> treatmentGroups) {
        if (treatmentGroups == null)
            throw new IllegalArgumentException("The treatmentGroups argument is required");
        EntityManager em = Subject.entityManager();
    ...
    public static TypedQuery<Subject> findSubjectsByCodeNameEquals(String codeName, String sortFieldName,
            String sortOrder) {
        if (codeName == null || codeName.length() == 0)
            throw new IllegalArgumentException("The codeName argument is required");
        EntityManager em = Subject.entityManager();
    ...
    public static TypedQuery<Subject> findSubjectsByLsTransactionEquals(Long lsTransaction) {
        if (lsTransaction == null)
            throw new IllegalArgumentException("The lsTransaction argument is required");
        EntityManager em = Subject.entityManager();
        TypedQuery<Subject> q = em.createQuery("SELECT o FROM Subject AS o WHERE o.lsTransaction = :lsTransaction",
                Subject.class);
        q.setParameter("lsTransaction", lsTransaction);
        return q;
    }

    public static TypedQuery<Subject> findSubjectsByLsTransactionEquals(Long lsTransaction, String sortFieldName,
            String sortOrder) {
        if (lsTransaction == null)
            throw new IllegalArgumentException("The lsTransaction argument is required");
        EntityManager em = Subject.entityManager();
        StringBuilder queryBuilder = new StringBuilder(
                "SELECT o FROM Subject AS o WHERE o.lsTransaction = :lsTransaction");
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            queryBuilder.append(" ORDER BY ").append(sortFieldName);
            if ("ASC".equalsIgnoreCase(sortOrder) || "DESC".equalsIgnoreCase(sortOrder)) {
                queryBuilder.append(" ").append(sortOrder);
