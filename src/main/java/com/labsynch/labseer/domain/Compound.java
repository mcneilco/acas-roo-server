...
public class Compound {
    ...
    public static Long countFindCompoundsByExternal_idEquals(String external_id) {
        if (external_id == null || external_id.length() == 0)
            throw new IllegalArgumentException("The external_id argument is required");
        EntityManager em = Compound.entityManager();
        TypedQuery q = em.createQuery("SELECT COUNT(o) FROM Compound AS o WHERE o.external_id = :external_id",
                Long.class);
    ...
    public static TypedQuery<Compound> findCompoundsByExternal_idEquals(String external_id) {
        if (external_id == null || external_id.length() == 0)
            throw new IllegalArgumentException("The external_id argument is required");
        EntityManager em = Compound.entityManager();
        TypedQuery<Compound> q = em.createQuery("SELECT o FROM Compound AS o WHERE o.external_id = :external_id",
                Compound.class);
        q.setParameter("external_id", external_id);
        return q;
    }

    public static TypedQuery<Compound> findCompoundsByExternal_idEquals(String external_id, String sortFieldName,
            String sortOrder) {
        if (external_id == null || external_id.length() == 0)
            throw new IllegalArgumentException("The external_id argument is required");
        EntityManager em = Compound.entityManager();
        StringBuilder queryBuilder = new StringBuilder(
                "SELECT o FROM Compound AS o WHERE o.external_id = :external_id");
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            queryBuilder.append(" ORDER BY ").append(sortFieldName);
            if ("ASC".equalsIgnoreCase(sortOrder) || "DESC".equalsIgnoreCase(sortOrder)) {
                queryBuilder.append(" ").append(sortOrder);
    ...
    public static final List<String> fieldNames4OrderClauseFilter = java.util.Arrays.asList("logger", "corpName",
            "external_id", "CdId", "createdDate", "modifiedDate", "ignored", "deleted");
    ...
    public static List<Compound> findCompoundEntries(int firstResult, int maxResults) {
        return entityManager().createQuery("SELECT o FROM Compound o", Compound.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }

    public static List<Compound> findCompoundEntries(int firstResult, int maxResults, String sortFieldName,
            String sortOrder) {
        String jpaQuery = "SELECT o FROM Compound o";
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            jpaQuery = jpaQuery + " ORDER BY " + sortFieldName;
        ...
        return entityManager().createQuery(jpaQuery, Compound.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }
