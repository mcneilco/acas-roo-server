...
public class StateType {
    ...
    public static List<StateType> findStateTypeEntries(int firstResult, int maxResults) {
        return entityManager().createQuery("SELECT o FROM StateType o", StateType.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }

    public static List<StateType> findStateTypeEntries(int firstResult, int maxResults, String sortFieldName,
            String sortOrder) {
        String jpaQuery = "SELECT o FROM StateType o";
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            jpaQuery = jpaQuery + " ORDER BY " + sortFieldName;
        ...
        return entityManager().createQuery(jpaQuery, StateType.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }
    ...
    public static TypedQuery<StateType> findStateTypesByTypeNameEquals(String typeName) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = StateType.entityManager();
        TypedQuery<StateType> q = em.createQuery("SELECT o FROM StateType AS o WHERE o.typeName = :typeName",
                StateType.class);
        q.setParameter("typeName", typeName);
        return q;
    }

    public static TypedQuery<StateType> findStateTypesByTypeNameEquals(String typeName, String sortFieldName,
            String sortOrder) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = StateType.entityManager();
