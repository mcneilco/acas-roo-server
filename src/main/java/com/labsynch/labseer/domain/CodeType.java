...
public class CodeType {
    ...
    public static TypedQuery<CodeType> findCodeTypesByTypeNameEquals(String typeName) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = CodeType.entityManager();
        TypedQuery<CodeType> q = em.createQuery("SELECT o FROM CodeType AS o WHERE o.typeName = :typeName",
                CodeType.class);
        q.setParameter("typeName", typeName);
        return q;
    }

    public static TypedQuery<CodeType> findCodeTypesByTypeNameEquals(String typeName, String sortFieldName,
            String sortOrder) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = CodeType.entityManager();
    ...
    public static List<CodeType> findCodeTypeEntries(int firstResult, int maxResults) {
        return entityManager().createQuery("SELECT o FROM CodeType o", CodeType.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }

    public static List<CodeType> findCodeTypeEntries(int firstResult, int maxResults, String sortFieldName,
            String sortOrder) {
        String jpaQuery = "SELECT o FROM CodeType o";
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            jpaQuery = jpaQuery + " ORDER BY " + sortFieldName;
        ...
        return entityManager().createQuery(jpaQuery, CodeType.class).setFirstResult(firstResult)
                .setMaxResults(maxResults).getResultList();
    }
