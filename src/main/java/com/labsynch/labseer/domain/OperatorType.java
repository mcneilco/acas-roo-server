...
public class OperatorType {
    ...
    @Id
    @SequenceGenerator(name = "operatorTypeGen", sequenceName = "OPERATOR_TYPE_PKSEQ", allocationSize = 1)
    ...
    public static Long countFindOperatorTypesByTypeNameEquals(String typeName) {
        ...
        TypedQuery q = em.createQuery("SELECT COUNT(o) FROM OperatorType AS o WHERE o.typeName = :typeName",
                Long.class);
    ...
    public static TypedQuery<OperatorType> findOperatorTypesByTypeNameEquals(String typeName) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = OperatorType.entityManager();
        TypedQuery<OperatorType> q = em.createQuery("SELECT o FROM OperatorType AS o WHERE o.typeName = :typeName",
                OperatorType.class);
        q.setParameter("typeName", typeName);
        return q;
    }

    public static TypedQuery<OperatorType> findOperatorTypesByTypeNameEquals(String typeName, String sortFieldName,
            String sortOrder) {
        if (typeName == null || typeName.length() == 0)
            throw new IllegalArgumentException("The typeName argument is required");
        EntityManager em = OperatorType.entityManager();
    ...
    public static List<OperatorType> findOperatorTypeEntries(int firstResult, int maxResults) {
        return entityManager().createQuery("SELECT o FROM OperatorType o", OperatorType.class)
                .setFirstResult(firstResult).setMaxResults(maxResults).getResultList();
    }

    public static List<OperatorType> findOperatorTypeEntries(int firstResult, int maxResults, String sortFieldName,
            String sortOrder) {
        String jpaQuery = "SELECT o FROM OperatorType o";
        if (fieldNames4OrderClauseFilter.contains(sortFieldName)) {
            jpaQuery = jpaQuery + " ORDER BY " + sortFieldName;
        ...
        return entityManager().createQuery(jpaQuery, OperatorType.class).setFirstResult(firstResult)
