...
public class LotDTO {
	...
	public LotDTO(Lot lot) {
		...
		if (lot.getModifiedBy() != null)
			this.modifiedBy = lot.getModifiedBy();
		...
		if (lot.getPhysicalState() != null)
			this.physicalStateCode = lot.getPhysicalState().getCode();
		if (lot.getProject() != null)
			this.project = lot.getProject();
		this.purity = lot.getPurity();
		if (lot.getPurityMeasuredBy() != null)
			this.purityMeasuredByCode = lot.getPurityMeasuredBy().getCode();
		if (lot.getPurityOperator() != null)
			this.purityOperatorCode = lot.getPurityOperator().getCode();
		this.registrationDate = lot.getRegistrationDate();
		this.retain = lot.getRetain();
		this.retainLocation = lot.getRetainLocation();
		if (lot.getRetainUnits() != null)
			this.retainUnitsCode = lot.getRetainUnits().getCode();
		if (lot.getRegisteredBy() != null)
			this.lotRegisteredBy = lot.getRegisteredBy();
		this.solutionAmount = lot.getSolutionAmount();
		if (lot.getSolutionAmountUnits() != null)
			this.solutionAmountUnitsCode = lot.getSolutionAmountUnits().getCode();
		...
		if (lot.getTareWeightUnits() != null)
			this.tareWeightUnitsCode = lot.getTareWeightUnits().getCode();
		this.totalAmountStored = lot.getTotalAmountStored();
		if (lot.getTotalAmountStoredUnits() != null)
			this.totalAmountStoredUnitsCode = lot.getTotalAmountStoredUnits().getCode();
		if (lot.getVendor() != null)
			this.vendorCode = lot.getVendor().getCode();
		if (!lot.getLotAliases().isEmpty()) {
			String lotAliases = "";
			for (LotAlias lotAlias : lot.getLotAliases()) {
				if (lotAliases.length() > 0)
					lotAliases += ";";
		...
		if (!saltForm.getIsoSalts().isEmpty()) {
			...
			for (IsoSalt isoSalt : saltForm.getIsoSalts()) {
				if (isoSaltAbbrevs.length() > 0)
					isoSaltAbbrevs += ";";
				if (isoSaltEquivalents.length() > 0)
					isoSaltAbbrevs += ";";
				if (isoSalt.getType().equalsIgnoreCase("salt"))
					isoSaltAbbrevs += isoSalt.getSalt().getAbbrev();
				else if (isoSalt.getType().equalsIgnoreCase("isotope"))
					isoSaltAbbrevs += isoSalt.getIsotope().getAbbrev();
		...
		if (parent.getStereoCategory() != null)
			this.parentStereoCategory = parent.getStereoCategory().getCode();
		...
		if (parent.getModifiedBy() != null)
			this.parentModifiedBy = parent.getModifiedBy();
		...
		if (parent.getParentAnnotation() != null)
			this.parentAnnotationCode = parent.getParentAnnotation().getCode();
		if (parent.getCompoundType() != null)
			this.parentCompoundTypeCode = parent.getCompoundType().getCode();
