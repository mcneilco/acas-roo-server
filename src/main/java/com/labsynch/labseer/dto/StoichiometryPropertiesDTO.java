...
public class StoichiometryPropertiesDTO {
	...
	public StoichiometryPropertiesDTO(LsThing thing) {
		LsThing parent = null;
		try {
			parent = ItxLsThingLsThing
					.findItxLsThingLsThingsByLsTypeEqualsAndLsKindEqualsAndFirstLsThingEquals("instantiates",
							"batch_parent", thing)
					.getSingleResult().getSecondLsThing();
		} catch (Exception e) {
			logger.debug("Could not find parent for provided lsThing. Continuing with batch properties");
		}
		// For all properties, try batch first, then try parent if property was not
		// found on batch
		...
		List<LsThingValue> molWeightValues = SimpleUtil.pluckValueByValueTypeKind(thing, "numericValue",
				"molecular weight");
		...
		if (molWeightValues.size() > 0) {
			...
			if (molWeightValues.size() > 0) {
				...
				List<LsThingValue> duplexMolWeightValues = SimpleUtil.pluckValueByValueTypeKind(parent, "numericValue",
				...
				if (molWeightValues.size() == 0 && duplexMolWeightValues.size() > 0) {
					...
					molWeightValues = SimpleUtil.pluckValueByValueTypeKind(parent, "numericValue",
		...
		List<LsThingValue> amountMadeValues = SimpleUtil.pluckValueByValueTypeKind(thing, "numericValue",
				"amount made");
