...
public class CurveFitDTO {
	...
	public CurveFitDTO(Map dataMap) {
		...
		if (dataMap.get("ec50Numeric") != null)
			this.ec50 = ((BigDecimal) dataMap.get("ec50Numeric")).toString();
		else
			this.ec50 = (String) dataMap.get("ec50String");
		if (dataMap.get("slopeNumeric") != null)
			this.slope = ((BigDecimal) dataMap.get("slopeNumeric")).toString();
		else
			this.slope = (String) dataMap.get("slopeString");
	...
	public CurveFitDTO(HashMap<String, String> stringMap, HashMap<String, BigDecimal> numericMap) {
		// These keys must be exactly the same as what is used in the database. Case
		// sensitive.
		...
		if (this.slope.equals("null"))
			this.slope = stringMap.get("Slope");
	...
	public static Collection<CurveFitDTO> getFitData(Collection<String> curveIds) {
		...
		if (curveIds.isEmpty())
			return new ArrayList<CurveFitDTO>();
	...
	public static CurveFitDTO getFitData(CurveFitDTO curveFitDTO) {
		...
		// AnalysisGroupValue.findAnalysisGroupValuesByLsState(doseResponseState).getResultList();
	...
	private static void saveFitData(AnalysisGroupState state, CurveFitDTO curveFitDTO) {
		...
		// Min, Max, EC50, and Slope are special cases. They must be filled in with
		// something for Seurat,
		...
		if (!(ec50 == null) && SimpleUtil.isDecimalNumeric(ec50)) {
			AnalysisGroupValue ec50Value = createCurveFitValue(state, "numericValue", "EC50", new BigDecimal(ec50),
					recordedBy, lsTransaction);
		...
		if (!(slope == null) && SimpleUtil.isDecimalNumeric(slope)) {
			AnalysisGroupValue slopeValue = createCurveFitValue(state, "numericValue", "Slope", new BigDecimal(slope),
					recordedBy, lsTransaction);
		...
		if (!(fittedEC50 == null)) {
			AnalysisGroupValue fittedEC50Value = createCurveFitValue(state, "numericValue", "Fitted EC50", fittedEC50,
					recordedBy, lsTransaction);
		...
		if (!(fittedSlope == null)) {
			AnalysisGroupValue fittedSlopeValue = createCurveFitValue(state, "numericValue", "Fitted Slope",
					fittedSlope, recordedBy, lsTransaction);
		...
		for (String curveId : curveIds) {
			try {
				...
				if (newRenderingHint != null) {
					if (!newRenderingHint.equals(renderingHint))
						logger.debug("Changing rendering hint from: " + renderingHint + " to: " + newRenderingHint);
	...
	private static Collection<ProtocolValue> getDisplayMinMaxByProtocolIds(Collection<Long> protocolIds) {
		...
		for (Long protocolId : protocolIds) {
			protocolValues
					.addAll(ProtocolValue.findProtocolValuesByProtocolIDAndStateTypeKindAndValueTypeKind(protocolId,
							"metadata", "screening assay", "numericValue", "curve display min").getResultList());
			protocolValues
					.addAll(ProtocolValue.findProtocolValuesByProtocolIDAndStateTypeKindAndValueTypeKind(protocolId,
							"metadata", "screening assay", "numericValue", "curve display max").getResultList());
		}
