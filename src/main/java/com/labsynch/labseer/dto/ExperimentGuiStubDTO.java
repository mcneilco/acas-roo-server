...
public class ExperimentGuiStubDTO {
	...
	public ExperimentGuiStubDTO(Experiment experiment) {
		...
		for (ExperimentState state : states) {
			if (state.getLsType().equals("metadata") && state.getLsKind().equals("experiment metadata")
					&& !state.isIgnored()) {
				Set<ExperimentValue> values = state.getLsValues();
				for (ExperimentValue value : values) {
					if (value.getLsType().equals("codeValue") && value.getLsKind().equals("scientist")
							&& !value.isIgnored()) {
						this.scientist = value.getCodeValue();
					} else if (value.getLsType().equals("dateValue") && value.getLsKind().equals("completion date")
							&& !value.isIgnored()) {
						this.completionDate = value.getDateValue();
					} else if (value.getLsType().equals("stringValue") && value.getLsKind().equals("status")
							&& !value.isIgnored()) {
						this.status = value.getStringValue();
					} else if (value.getLsType().equals("stringValue") && value.getLsKind().equals("analysis status")
							&& !value.isIgnored()) {
						this.analysisStatus = value.getStringValue();
					} else if (value.getLsType().equals("stringValue") && value.getLsKind().equals("notebook")
							&& !value.isIgnored()) {
						this.notebook = value.getStringValue();
					} else if (value.getLsType().equals("stringValue") && value.getLsKind().equals("notebook page")
							&& !value.isIgnored()) {
						this.notebookPage = value.getStringValue();
					} else if (value.getLsType().equals("clobValue") && value.getLsKind().equals("long description")
							&& !value.isIgnored()) {
