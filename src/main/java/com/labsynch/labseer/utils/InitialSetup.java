	...
	private static void setupExperimentTypes() throws IOException {
		...
		if (ExperimentType.countExperimentTypes() < 1) {
			...
			for (ExperimentType experimentType : experimentTypes) {
				if (ExperimentType.findExperimentTypesByTypeNameEquals(experimentType.getTypeName())
						.getMaxResults() == 0) {
	...
	private static void setupExperimentKinds() throws IOException {
		if (ExperimentKind.countExperimentKinds() < 1) {
			...
			for (ExperimentKind experimentKind : experimentKinds) {
				List<ExperimentType> dbLsTypes = ExperimentType
						.findExperimentTypesByTypeNameEquals(experimentKind.getLsType().getTypeName()).getResultList();
				if (dbLsTypes.size() == 1) {
	...
	private static void setupProtocolKinds() throws IOException {
		...
		if (ProtocolKind.countProtocolKinds() < 1) {
			...
			for (ProtocolKind protocolKind : protocolKinds) {
				List<ProtocolType> dbLsTypes = ProtocolType
						.findProtocolTypesByTypeNameEquals(protocolKind.getLsType().getTypeName()).getResultList();
				if (dbLsTypes.size() == 1) {
	...
	private static void setupThingKinds() throws IOException {
		...
		if (ThingKind.countThingKinds() < 1) {
			...
			for (ThingKind thingKind : thingKinds) {
				List<ThingType> dbLsTypes = ThingType
						.findThingTypesByTypeNameEquals(thingKind.getLsType().getTypeName()).getResultList();
				if (dbLsTypes.size() == 1) {
	...
	private static void setupLabelKinds() throws IOException {
		...
		if (LabelKind.countLabelKinds() < 1) {
			...
			for (LabelKind labelKind : labelKinds) {
				List<LabelType> dbLsTypes = LabelType
						.findLabelTypesByTypeNameEquals(labelKind.getLsType().getTypeName()).getResultList();
				if (dbLsTypes.size() == 1) {
