			...
			while ((mol = mi.readNextMol()) != null) {
				...
				if (MoleculeUtil.validateMolProperty(mol, "Date")) {
					try {
						...
						logger.error(
								"error parsing the date. Date field = " + MoleculeUtil.getMolProperty(mol, "Date"));
						mol.setProperty("error",
								"unable to parse date field.  " + MoleculeUtil.getMolProperty(mol, "Date"));
						goodMolToProcess = false;
					}
				}
				...
				try {
					...
					if (MoleculeUtil.validateMolProperty(mol, "StereoCategory")) {
						...
						try {
							stereoCategory = StereoCategory.findStereoCategorysByCodeEquals(stereoCategoryCode)
									.getSingleResult();
						} catch (NoResultException e2) {
							logger.error("Did not find the stereoCategory code" + stereoCategoryCode);
							stereoCategory = StereoCategory.findStereoCategorysByCodeEquals("unknown")
									.getSingleResult();
						}
					} else {
					...
					if (MoleculeUtil.validateMolProperty(mol, "RemovedSalts")) {
						...
						if (!saltAbbrev.equalsIgnoreCase("")) {
							...
							try {
								...
								logger.debug(
										"Did not find the salt code " + saltAbbrev + "  Check if it is an isotope.");
								...
								try {
									...
									logger.error(
											"Not an isotope either!!. Did not find the isotope code " + saltAbbrev);
				...
				String lotCorpName = saltForm.getCorpName().concat(propertiesUtilService.getCorpSeparator())
						.concat(String.format("%02d", lotNumber));
