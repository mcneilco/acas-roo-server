...
privileged aspect UnitKindController_Roo_Controller {
    ...
    public String UnitKindController.createForm(Model uiModel) {
        ...
        if (UnitType.countUnitTypes() == 0) {
            dependencies.add(new String[] { "unittype", "unittypes" });
        }
    ...
    public String UnitKindController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("unitkinds", UnitKind.findUnitKindEntries(firstResult, sizeNo));
            float nrOfPages = (float) UnitKind.countUnitKinds() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("unitkinds", UnitKind.findAllUnitKinds());
        }
