...
privileged aspect ProtocolKindController_Roo_Controller {
    ...
    public String ProtocolKindController.createForm(Model uiModel) {
        ...
        if (ProtocolType.countProtocolTypes() == 0) {
            dependencies.add(new String[] { "protocoltype", "protocoltypes" });
        }
    ...
    public String ProtocolKindController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("protocolkinds", ProtocolKind.findProtocolKindEntries(firstResult, sizeNo));
            float nrOfPages = (float) ProtocolKind.countProtocolKinds() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("protocolkinds", ProtocolKind.findAllProtocolKinds());
        }
